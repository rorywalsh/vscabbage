...
<Cabbage>
form caption("Oscilloscope") size(470, 375), guiMode("queue"), pluginId("OsSc"), colour("DarkSlateGrey")

; OSCILLOSCOPE PARAMETERS
; bevel
image         bounds(  5,  5, 390, 230), colour(0,0,0,0), outlineThickness(10), outlineColour("Silver"), corners(20)
; grid - this is an overlay. The oscilloscope does not include a grid. alpha() for the signaldisplay needs to be less that 1 for this to be visible.
gentable      bounds( 10, 10, 380, 220), tableNumber(1),  tableGridColour("white")
; oscilloscope
signaldisplay bounds( 10, 10, 380, 220), colour(150,255,150), outlineThickness(1.5) alpha(0.85), displayType("waveform"), backgroundColour("Black"), zoom(-1), signalVariable("aSig", "a2"), channel("display")
...
rslider bounds(  0, 25, 80, 80), channel("OscAmp"), range(0,1,0.4,0.5), text("Osc.Amp."), valueTextBox(1), fontColour("White"), textColour("White")
rslider bounds( 75, 25, 80, 80), channel("OscFreq"), range(20,8000,80,0.5,1), text("Osc.Freq."), valueTextBox(1), fontColour("White"), textColour("White")
...
label    bounds(  5,361,110, 12), text("Iain McCurdy |2023|"), align("left"), fontColour("white")
...
kPrdMode, kTPrdMode cabbageGetValue    "PrdMode" ; period mode 1=free 2=sync

kSource             cabbageGetValue    "Source"

if kTPrdMode==1 then ; free
 if kPrdMode==1 then
...
; in sync mode, period of the display sync with the frequency of the oscillator
