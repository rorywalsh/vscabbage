...
; 
; In this example the user can select from a number of data sets of partials using a combobox.
...
; 
; The GEN33 table can be auditioned and interacted with manually using the controls in the first groupbox
...
<Cabbage>
form caption("GEN33"), size(520, 334), pluginId("gn33"), guiMode("queue"), colour(40,40,40)
gentable    bounds(  0,  0, 520, 120), channel("table1"), tableNumber(1), tableColour("yellow"), zoom(-1), , fill(0), ampRange(1,-1,1)
...
label    bounds(  2,321,110, 12), text("Iain McCurdy |2014|"), fontColour("silver"), align("left")
...
instr    1    ; manual instrument
 gknh           cabbageGetValue    "nh"            ; read in widgets
 gkamp          cabbageGetValue    "amp"
 gkfrq          cabbageGetValue    "frq"
 gkManualOnOff  cabbageGetValue    "ManualOnOff"
 ...
 UPDATE_WAVEFORM:                                                    ; begin reinitialisation from here
  ...
  iNumPartials  =                  ftlen(gisource)/3
                cabbageSetValue    "NumPartials", iNumPartials       ; write number of partials for this algorithm to number box
...
instr    3    ; midi / keyboard instrument
 iDecTim        cabbageGetValue    "DecTim"
