...
<Cabbage>
form caption("GEN42"), size(420,507), pluginId("gn42"), colour(50, 40, 40), guiMode("queue")

gentable bounds(  5,  5, 410, 145), channel("table1"), tableNumber(1), outlineThickness(2), tableColour("yellow"), ampRange(24,96,1), zoom(-1), fill(0)

image    bounds(  5,  5, 1, 145), channel("Scrubber"), alpha(0.5)
...
checkbox bounds( 15, 50,115, 17), channel("SynthOnOff"), text("On/Off"),  value(1), colour:1("yellow"), colour:0("black"), shape("square")
...
rslider  bounds(125, 25, 70, 70), channel("rate"), text("Rate"), valueTextBox(1), textBox(1),  textBox(1), range(0.2,20,2,0.5,0.01), colour(210,110, 80,200), trackerColour("white")
rslider  bounds(175, 25, 70, 70), channel("jit"),  text("Jitter"), valueTextBox(1),  textBox(1), range(0, 1, 0), colour(210,110, 80,200), trackerColour("white")
rslider  bounds(225, 25, 70, 70), channel("vel"),  text("Velocity"), valueTextBox(1),  textBox(1), range(0, 1, 0), colour(210,110, 80,200), trackerColour("white")
rslider  bounds(275, 25, 70, 70), channel("dur"),  text("Dur."), valueTextBox(1),  textBox(1), range(0.01, 3.00, 2), colour(210,110, 80,200), trackerColour("white")
nslider  bounds(345, 40, 60, 30), channel("note"),  text("note"), range(0, 127, 0, 1, 0.01), colour(120,170,200, 50)
}

label    bounds( 5,495,110, 12), text("Iain McCurdy |2014|"), fontColour("silver"), align("left")
...
instr    1
 ...
 ; read in widgets
 kmin1             cabbageGetValue     "min1"
 kmin2             cabbageGetValue     "min2"
 kmin3             cabbageGetValue     "min3"
 kmin4             cabbageGetValue     "min4"
 kmin5             cabbageGetValue     "min5"
 kmin6             cabbageGetValue     "min6"
 kmin7             cabbageGetValue     "min7"
 kmin8             cabbageGetValue     "min8"

 kmax1             cabbageGetValue     "max1"
 kmax2             cabbageGetValue     "max2"
 kmax3             cabbageGetValue     "max3"
 kmax4             cabbageGetValue     "max4"
 kmax5             cabbageGetValue     "max5"
 kmax6             cabbageGetValue     "max6"
 kmax7             cabbageGetValue     "max7"
 kmax8             cabbageGetValue     "max8"
 ...
 if ktrig==1 then    ; slow rate of update. Tables updated at this rate. If too slow, glitching will be heard in the output, particularly if random movement speed is high. If too high CPU performance will suffer.
                   reinit              UPDATE
 endif
 UPDATE:
...
instr    2
 iNdx              random              0, 1                                ; generate a random value
                   cabbageSet          "Scrubber", "bounds",  5+iNdx*410,  5, 1, 145                     ; send position message to widget
