...
; This is a simple demonstration of GEN32.
; The only difference between GEN31 and GEN32 is that GEN32 allows the initial phase of each of the components to be controlled.
;  In this example, this parameter is controlled using the tiny dials underneath the polarity buttons in the 'O U T P U T   W A V E F O R M' section.
; It is recommended to first gain an understanding of GEN09 as GEN32 is an extension of its functionality.
...
; GEN32 operates in the same way except that the input waveform need not be a sine wave but can be any waveform created using any other GEN routine.
...
; Therefore GEN32 can be thought of as creating a harmonic series of harmonic waveforms. With some settings and if the frequency of the wave sonification is lowish,
;  the interesting situation of storing a chord in a single function table can be perceived.
; Note that the waveform used by each partial of the GEN32 wave need not be the same. 
...
; Sonification is a simple oscillator. Auditioning of the input waveform and GEN32 output can be selected.
...
<Cabbage>
form caption("GEN32"), size(710, 472), pluginId("gn18"), colour("silver"), guiMode("queue")
...
vslider   bounds( 10, 25, 20, 47), channel("OP1"), range(0,1,0,0.5)
vslider   bounds( 30, 25, 20, 47), channel("OP2"), range(0,1,1,0.5)
vslider   bounds( 50, 25, 20, 47), channel("OP3"), range(0,1,0,0.5)
vslider   bounds( 70, 25, 20, 47), channel("OP4"), range(0,1,1,0.5)
vslider   bounds( 90, 25, 20, 47), channel("OP5"), range(0,1,0,0.5)
vslider   bounds(110, 25, 20, 47), channel("OP6"), range(0,1,0,0.5)
vslider   bounds(130, 25, 20, 47), channel("OP7"), range(0,1,0,0.5)
vslider   bounds(150, 25, 20, 47), channel("OP8"), range(0,1,0,0.5)

nslider   bounds( 10, 65, 20, 20), channel("OPN1"), range(1,99,1,1,1)
nslider   bounds( 30, 65, 20, 20), channel("OPN2"), range(1,99,2,1,1)
nslider   bounds( 50, 65, 20, 20), channel("OPN3"), range(1,99,3,1,1)
nslider   bounds( 70, 65, 20, 20), channel("OPN4"), range(1,99,4,1,1)
nslider   bounds( 90, 65, 20, 20), channel("OPN5"), range(1,99,5,1,1)
nslider   bounds(110, 65, 20, 20), channel("OPN6"), range(1,99,6,1,1)
nslider   bounds(130, 65, 20, 20), channel("OPN7"), range(1,99,7,1,1)
nslider   bounds(150, 65, 20, 20), channel("OPN8"), range(1,99,8,1,1)

checkbox   bounds( 15, 85, 10,  10), channel("ONeg1"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)
checkbox   bounds( 35, 85, 10,  10), channel("ONeg2"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)
checkbox   bounds( 55, 85, 10,  10), channel("ONeg3"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)
checkbox   bounds( 75, 85, 10,  10), channel("ONeg4"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)
checkbox   bounds( 95, 85, 10,  10), channel("ONeg5"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)
checkbox   bounds(115, 85, 10,  10), channel("ONeg6"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)
checkbox   bounds(135, 85, 10,  10), channel("ONeg7"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)
checkbox   bounds(155, 85, 10,  10), channel("ONeg8"), text("",""), shape("ellipse") colour:0(60,0,0), colour:1(255,100,100)

rslider   bounds( 10, 95, 20, 20), channel("OPh1"), range(-1,1,0)
rslider   bounds( 30, 95, 20, 20), channel("OPh2"), range(-1,1,0)
rslider   bounds( 50, 95, 20, 20), channel("OPh3"), range(-1,1,0)
rslider   bounds( 70, 95, 20, 20), channel("OPh4"), range(-1,1,0)
rslider   bounds( 90, 95, 20, 20), channel("OPh5"), range(-1,1,0)
rslider   bounds(110, 95, 20, 20), channel("OPh6"), range(-1,1,0)
rslider   bounds(130, 95, 20, 20), channel("OPh7"), range(-1,1,0)
rslider   bounds(150, 95, 20, 20), channel("OPh8"), range(-1,1,0)
...
label    bounds(  4,  4, 60, 14), text("GEN32"), align("left")
image    bounds(  2, 60,396,  1), colour(255,255,255,100) ; X AXIS
}
...
instr    1
 ...
 kOPh1             cabbageGetValue     "OPh1"
 kOPh2             cabbageGetValue     "OPh2"
 kOPh3             cabbageGetValue     "OPh3"
 kOPh4             cabbageGetValue     "OPh4"
 kOPh5             cabbageGetValue     "OPh5"
 kOPh6             cabbageGetValue     "OPh6"
 kOPh7             cabbageGetValue     "OPh7"
 kOPh8             cabbageGetValue     "OPh8"
 
  ; output waveform
 if changed:k(kP1,kP2,kP3,kP4,kP5,kP6,kP7,kP8,kPN1,kPN2,kPN3,kPN4,kPN5,kPN6,kPN7,kPN8,kOP1,kOP2,kOP3,kOP4,kOP5,kOP6,kOP7,kOP8,kOPN1,kOPN2,kOPN3,kOPN4,kOPN5,kOPN6,kOPN7,kOPN8,kOPGraphGain,kNParts,kLPart,kPow,ktrig,kONeg1,kONeg2,kONeg3,kONeg4,kONeg5,kONeg6,kONeg7,kONeg8,kINeg1,kINeg2,kINeg3,kINeg4,kINeg5,kINeg6,kINeg7,kINeg8,kNorm,kGEN08,kGEN09,kGEN11,kOPh1,kOPh2,kOPh3,kOPh4,kOPh5,kOPh6,kOPh7,kOPh8)==1 then
  reinit REBUILD_OUTPUT_WAVEFORM
 endif
 REBUILD_OUTPUT_WAVEFORM:
  i_               ftgen               giOWfm, 0, ftlen(giOWfm), 32*i(kNorm), giIWfm, i(kOPN1), i(kOP1)*i(kONeg1),i(kOPh1), giIWfm, i(kOPN2), i(kOP2)*i(kONeg2),i(kOPh2), giIWfm, i(kOPN3), i(kOP3)*i(kONeg3),i(kOPh3), giIWfm, i(kOPN4), i(kOP4)*i(kONeg4),i(kOPh4), giIWfm, i(kOPN5), i(kOP5)*i(kONeg5),i(kOPh5), giIWfm, i(kOPN6), i(kOP6)*i(kONeg6),i(kOPh6), giIWfm, i(kOPN7), i(kOP7)*i(kONeg7),i(kOPh7), giIWfm, i(kOPN8), i(kOP8)*i(kONeg8),i(kOPh8), giIWfm, i(kOPN8), i(kOP8)*i(kONeg8),i(kOPh8) ; needs an extra 'dummy' term...
