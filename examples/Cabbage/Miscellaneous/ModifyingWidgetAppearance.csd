...
In this example various attributes of a 'image' widget can be modified using the on-screen sliders comboboxes and buttons. For the sake of efficiency widgets are only sent updates when the sliders are moved. We can detect changes in the sliders using triggers generated by the cabbageGetValue opcode, which will retrieve the value of the slider.
  ...
  kHorPos             cabbageGetValue       "HorPos"
  kVertPos            cabbageGetValue       "VertPos"
  kSizeX              cabbageGetValue       "SizeX"
  kSizeY              cabbageGetValue       "SizeY"
                      cabbageSet            changed:k(kHorPos,kVertPos,kSizeX,kSizeY), "image1", "bounds", kHorPos, 200-kVertPos, kSizeX, kSizeY
  
  ; Change shape
  SShape              cabbageGet            "Shape"
                      cabbageSet            changed:k(SShape), "image1", "shape", SShape

  ; Visibility
  kVisibleInvisible   cabbageGet            "VisibleInvisible"
                      cabbageSet            changed:k(kVisibleInvisible), "image1", "visible", kVisibleInvisible-1
  
  ; Change line thickness
  kLine               cabbageGet            "Line"
                      cabbageSet            changed:k(kLine), "image1", "outlineThickness", kLine

  ; Change corner size
  kcornersVal         cabbageGet            "cornersVal"
                      cabbageSet            changed:k(kcornersVal), "image1", "corners", kcornersVal
  
  ; Change line colour
  kLineRed            cabbageGetValue       "LineRed"
  kLineGreen          cabbageGetValue       "LineGreen"
  kLineBlue           cabbageGetValue       "LineBlue"
  kLineAlpha          cabbageGetValue       "LineAlpha"   
                      cabbageSet   changed:k(kLineRed,kLineGreen,kLineBlue,kLineAlpha), "image1", "outlineColour", kLineRed, kLineGreen, kLineBlue, kLineAlpha
  
  ; Change colour
  kRed                cabbageGetValue       "Red"
  kGreen              cabbageGetValue       "Green"
  kBlue               cabbageGetValue       "Blue"
  kAlpha              cabbageGetValue       "Alpha" 
                      cabbageSet            changed:k(kRed,kGreen,kBlue,kAlpha), "image1", "colour", kRed, kGreen, kBlue, kAlpha
  
  ; Change rotation
  kradians            cabbageGetValue       "radians"
  kpivotx             cabbageGetValue       "pivotx"
  kpivoty             cabbageGetValue       "pivoty"
                      cabbageSet            changed:k(kradians,kpivotx,kpivoty), "image1", "rotate", kradians, kpivotx*kSizeX, kpivoty*kSizeY
