...
<Cabbage>
form caption("Time Bandit") size(945, 507), pluginId("TiBn") colour(20,25,30) guiMode("queue")
...
soundfiler bounds( 20, 17, 915, 200), channel("beg","len"), channel("filer1"),  colour(255, 100, 100), fontColour(160, 160, 160, 255)
gentable   bounds( 20, 15, 915, 200), tableNumber(101), tableColour:0(155,155,255,255), tableBackgroundColour(0,0,0,0), fill(0), outlineThickness(2), tableGridColour(0,0,0,0), channel("IDSpeedGraph"), ampRange(0,1,-1,0.01), zoom(-1), active(1)
image      bounds( 20, 17,   1, 200), channel("scrubber")
label      bounds( 20, 18, 915, 14), text(""), align("left"), colour(0,0,0,0), fontColour(200,200,200), channel("stringbox")
line       bounds( -20, -85, 915, 1), channel("IDUnisonLine")

vslider    bounds( 0,   9,20, 219), channel("UniLine"), range(0, 1, 0.5),  colour(250,230,250,250), trackerColour("silver")

combobox   bounds( 10,223, 90, 25), channel("Mode"), items("Varispeed", "Timestretch", "Pitch Shift", "Filter", "Vari/Filter"), value(1), fontColour("white")

image      bounds( 10,255,100,115), colour(0,0,0,0), plant("controls"), outlineColour("silver"), outlineThickness(1) 
{
...
image      bounds(115,255,160,115), colour(0,0,0,0), plant("Range"), outlineColour("silver"), outlineThickness(1) 
{
...
image      bounds(280,255, 80,115), colour(0,0,0,0), plant("Quality"), outlineColour("silver"), outlineThickness(1) {
...
image      bounds(365,255,300,115), colour(0,0,0,0), plant("Filter"), outlineColour("silver"), outlineThickness(1) {
...
image      bounds(670,255,265,115), colour(0,0,0,0), plant("RandomLFO"), outlineColour("silver"), outlineThickness(1) {
...
image      bounds( 10,375,460,115), colour(0,0,0,0), plant("Follow"), outlineColour("silver"), outlineThickness(1) {
...
image      bounds(475,375,460,115), colour(0,0,0,0), plant("Follow"), outlineColour("silver"), outlineThickness(1) {
...
label    bounds( 10,492,120, 12), text("Iain McCurdy |2021|"), align("left"), fontColour("Silver")
...
giSpeedGraph    ftgen       101, 0,   16384, -5, 0.5, 16384, 0.5
...
instr    1
 ...
 gkUniLine     cabbageGetValue    "UniLine"
 kMinSpd       cabbageGetValue    "MinSpd"
 kMaxSpd       cabbageGetValue    "MaxSpd"
 gkRange       cabbageGetValue    "Range"
 gkRate        cabbageGetValue    "Rate"
 gkLock        cabbageGetValue    "Lock"
 gkRecOut      cabbageGetValue    "RecOut"
 gkBaseFilt    cabbageGetValue    "BaseFilt"
 gkMaxFilt     cabbageGetValue    "MaxFilt"
 gkShapeFilt   cabbageGetValue    "ShapeFilt"
 gkNPoles      cabbageGetValue    "NPoles"
 gkListenAhead cabbageGetValue    "ListenAhead"
 gkAttack      cabbageGetValue    "Attack"
 gkRelease     cabbageGetValue    "Release"
 gkFollowShape cabbageGetValue    "FollowShape"
 gkFollowSens  cabbageGetValue    "FollowSens"

gkListenAheadTrig cabbageGetValue "ListenAheadTrig"
gkTimeTrig        cabbageGetValue "TimeTrig"
gkThreshTrig      cabbageGetValue "ThreshTrig"
gkWaitTrig        cabbageGetValue "WaitTrig"
gkSensTrig        cabbageGetValue "SensTrig"
 ...
 UPDATE_UNILINE:
  ...
  ;              chnset           Smsg, "IDUnisonLine"
                 cabbageSet       k(1),"IDUnisonLine","bounds",20, (1-i(gkUniLine))*200 + 15, 915, 1
 ...
 kRandomise      cabbageGetValue "Randomise"
 if trigger:k(kRandomise,0.5,0)==1 then
                 event           "i", 10, 0, 0.1
 endif

 kLFO            cabbageGetValue "LFO"
 if trigger:k(kLFO,0.5,0)==1 then
                 event           "i", 11, 0, 0.1
 endif

 kFollow         cabbageGetValue "Follow"
 if trigger:k(kFollow,0.5,0)==1 then
                 event           "i", 12, 0, -1
                 cabbageSetValue "Follow",k(0),k(1)
 endif

 kTrigger        cabbageGetValue "Trigger"
 if trigger:k(kTrigger,0.5,0)==1 then
                 event           "i", 13, 0, -1
                 cabbageSetValue "Trigger",k(0),k(1)
 endif

 if trigger:k(gkRecOut,0.5,0)==1 then
                 cabbageSetValue "PlayStop",k(1),k(1)
                 event           "i", 1000, 0, -1
 endif
...
; CALCULATION FOR SCRUBBER SEEMS OFF
 kNdx        +=               (ftlen(giSpeedGraph)  /   (iLen * kr)) * kSpd
             cabbageSet       k(1), "filer1","scrubberPosition", iLen * sr * kNdx/ftlen(giSpeedGraph)
;             printk 0.1,kNdx
;soundfiler bounds( 20, 17, 915, 200), channel("beg","len"), channel("filer1"),  colour(255, 100, 100), fontColour(160, 160, 160, 255)
;gentable   bounds( 20, 15, 915, 200), tableNumber(101), tableColour:0(155,155,255,255), tableBackgroundColour(0,0,0,0), fill(0), outlineThickness(2), tableGridColour(0,0,0,0), channel("IDSpeedGraph"), ampRange(0,1,-1,0.01), zoom(-1), active(1)
;image      bounds( 20, 17, 915, 200), channel("scrubber")

 ;if kNdx>ftlen(giSpeedGraph) then
 ;            cabbageSetValue "PlayStop",k(0), k(1)
 ;            turnoff
 ;endif
...
instr    3
 ...
 ;scrubber
             cabbageSet k(1), "filer1", "scrubberPosition", kPnt * sr
...
instr    4
 ...
 ;scrubber
             cabbageSet k(1), "filer1", "scrubberPosition", k(aPnt)*sr
...
instr 10
 ...
 od
             ...
             cabbageSet k(1),"IDSpeedGraph","tableNumber",101
             turnoff
endin
...
instr 1000
  a1, a2         monitor
  SHome          cabbageGetValue    "USER_HOME_DIRECTORY"
  ...
  if active:k(1+i(gkMode))==0 && timeinstk()>1 then
   cabbageSetValue "RecOut",k(0),k(1)
   turnoff
  endif
