...
; MIDI
; Pitch bend can also be used to control the Whammy effect
; Modulation Wheel controls the sustain

<Cabbage>
form caption("String Feedback"), size(885, 225), pluginId("fbck"), colour(0,0,0), guiMode("queue")
...
label    bounds( 5,212,110,11), text("Iain McCurdy |2013|"), fontColour("grey"), align("left")
...
gkModWhl          =                (gkModWhl/127)^3
                  cabbageSetValue  "sustain", (gkModWhl*0.9999)+0.0001, changed:k(gkModWhl)
                  cabbageSetValue  "fback", gkModWhl*8, changed:k(gkModWhl)
endif

kporttime         linseg           0,0.001,0.03       ; used to smooth GUI control movement
...
ksustain          cabbageGetValue  "sustain"          ; string sustain (inverse of damping). Higher values = more sustain
...
; pick-up position
kauto             cabbageGetValue  "auto"                                       ; 'auto' checkbox button.
if(kauto==0)      then                                                          ; if 'auto' button is off pick-up position is set manually
 kpick            cabbageGetValue  "PickPos"                                    ; 'Position' GUI knob
 ...
 kPickposSpeed    cabbageGetValue  "PickPosSpeed"                               ; pick-up position modulation 'Speed' GUI knob
...
irel              cabbageGetValue  "rel"                     ; feedback signal release time
...
; overdrive
kDrive            cabbageGetValue  "drive"              ; overdrive value from GUI knob
...
; filtering
kLPF              cabbageGetValue  "LPF"                   ; Lowpass filter ('Bright') GUI knob. This is a ratio of the fundemental frequency so the final cutoff frequency will depend on the note played (keyboard tracking).
...
kHPF              cabbageGetValue  "HPF"                   ; same as the above but with a highpass filter. (Highpass filter called 'Cut' in GUI.)
kHPF              portk            kHPF, kporttime
kHPF              limit            icps * kHPF, 1, sr/2
asig              buthp            asig, kHPF

; amplitude enveloping
iStrRel           cabbageGetValue  "StrRel"
...
kspeed            cabbageGetValue  "speed"                             ; delay time modulation speed
...
klevel            cabbageGetValue  "level"            ; level control (pre reverb input therefore allows reverb to die away naturally)
...
kMOUSE_DOWN_LEFT  cabbageGetValue  "MOUSE_DOWN_LEFT"
                  cabbageSetValue  "Bend",0,trigger:k(kMOUSE_DOWN_LEFT,0.5,1)
