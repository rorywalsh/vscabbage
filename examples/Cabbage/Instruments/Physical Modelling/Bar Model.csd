...
<Cabbage>
form caption("Bar Model"), size(955,340), pluginId("BaMo"), colour(0,0,0), guiMode("queue")
...
image     bounds(505, 15, 90,  1), colour(30,30,30), channel("AttackLine")
label     bounds(525,  9, 50, 12), text("Attack"), colour(0,0,0), fontColour(30,30,30), channel("AttackLabel")
...
image     bounds(775, 15, 90,  1), colour(30,30,30), channel("ReverbLine")
label     bounds(795,  9, 50, 12), text("Reverb"), colour(0,0,0), fontColour(30,30,30), channel("ReverbLabel")
...
groupbox  bounds(  5,115,400,135), text("Velocity Mappings"), colour(0,0,0,60), shape("sharp"), plant("VelocityMappings")
{
label     bounds(  5, 25,120, 10), text("Strike Position")
gentable  bounds(  5, 37,120, 90), tableNumber(2), channel("table2"), ampRange(0,1,2), zoom(-1), tableColour("turquoise"), active(1), fill(0)
label     bounds(135, 25,120, 10), text("Tone")
gentable  bounds(135, 37,120, 90), tableNumber(3), channel("table3"), ampRange(0,1,3), zoom(-1), tableColour("turquoise"), active(1), fill(0)
label     bounds(265, 25,120, 10), text("Amplitude")
gentable  bounds(265, 37,120, 90), tableNumber(4), channel("table4"), ampRange(0.01,1,4), zoom(-1), tableColour("turquoise"), active(1), fill(0)
}

; MODULATION
groupbox  bounds(410,115,540,135), text("Modulation"), colour(0,0,0,60), shape("sharp"), plant("modulation")
{
...
gentable  bounds(150, 37,120, 90), tableNumber(5), channel("table5"), ampRange(0.01,2,-1), zoom(-1), tableColour("yellow"), active(1), fill(0)
...
label bounds(440, 122,98,10),text("Iain McCurdy 2015")
}

keyboard bounds(5, 255, 945, 80) mouseOverKeyColour(255, 255, 0, 128) 
...
instr    1    ; always on instrument. Read in widgets that will be used as global variables.
 ...
 gkscan           cabbageGetValue  "scan"        
 kscan_x100       cabbageGetValue  "scan_x100"
 if kscan_x100==1 then
  gkscan          *=               100
 endif
 gkScanMix        cabbageGetValue  "ScanMix" 
 gkOutAmp         cabbageGetValue  "OutAmp"
 gkOutAmp         portk            gkOutAmp,kporttime
 gkRndModAmt      cabbageGetValue  "RndModAmt"
 gkRndModRte      cabbageGetValue  "RndModRte"
 gkPanDepth       cabbageGetValue  "PanDepth"
 gkRvbMix         cabbageGetValue  "RvbMix"
 gkRvbSize        cabbageGetValue  "RvbSize"
 gkDry            limit            (1-gkRvbMix)*2,0,1
 gkWet            limit            gkRvbMix*2,0,1
 gkKybdScal       cabbageGetValue  "KybdScal"
 
; ; TUNING
; ; TEST TONE
; gknote          init             60                             ; (used for diagnostics and tuning)
; gktrans         cabbageGetValue  "trans"                        ; (used for diagnostics and tuning)
; kTestTone       cabbageGetValue  "TestTone"                     ; (used for diagnostics and tuning)
; if kTestTone==1 then                                            ; (used for diagnostics and tuning)
;  kTestVol       cabbageGetValue  "TestVol"                      ; (used for diagnostics and tuning)
...
; gkoffset        cabbageGetValue  "offset"                        ; (used for diagnostics and tuning)
...
; 
; ; ZERO OFFSET
; kZeroOffset     cabbageGetValue  "ZeroOffset"      ; (used for diagnostics and tuning)
...
instr    2    ; receive midi note ons and offs
 ...
 ibc              cabbageGetValue  "bc"                        ; Boundary Conditions selection (1,2,3 or 4)
 ...
 if inote>=iNoteMin && inote<=iNoteMax then
    ...
    gkNoteTable[inote]    =        1
  giPolyLimit     cabbageGetValue  "PolyLimit"
...
; gknote          init             inote              ; (used for diagnostics and tuning)
 ...
 ib               cabbageGetValue  "b"                ; damping
 ib               /=               iDurRatio
 irel             cabbageGetValue  "rel"              ; release time
 iT30             cabbageGetValue  "T30"              ; 30dB decay time
 iT30             *=               iDurRatio
 iT30_Hold        cabbageGetValue  "T30_Hold"         ; over-ride decay slider value (HOLD button)
 iT30             =                iT30_Hold == 1 ? 3600 : iT30    ; if 'HOLD' button is active set T30 value to 3600 and ignore slider value
 ibc              cabbageGetValue  "bc"               ; boundary conditions selection (1,2,3 or 4)
 iwid             cabbageGetValue  "wid"              ; strike width
 ...
 ;ioffset         cabbageGetValue  "offset"            ; (used for diagnostics and tuning)
 ;iK              =                iK + ioffset        ; (used for diagnostics and tuning)
                  cabbageSetValue  "output",iK
 ...
 ; MODULATION ENVELOPE
 iModEnvDur       cabbageGetValue  "ModEnvDur"                ; envelope duration
 ...
 iAttAmt          cabbageGetValue  "AttAmt"        ; attack amount
 iAttTim          cabbageGetValue  "AttTim"        ; attack time
 aatt             init             1
...
instr    80    ; reverb
 ...
 if kOnTrig==1 then
                cabbageSet         1,"ReverbLabel","fontColour",170,170,170
                cabbageSet         1,"ReverbLine","colour",170,170,170
 elseif kOffTrig==1 then
                cabbageSet         1,"ReverbLabel","fontColour",30,30,30
                cabbageSet         1,"ReverbLine","colour",30,30,30
 endif

 gkAttTim       cabbageGetValue    "AttTim"
 kOnTrig        trigger            gkAttTim,0.0005,0
 kOffTrig       trigger            gkAttTim,0.0005,1
 if kOnTrig==1 then
                cabbageSet         1,"AttackLabel","fontColour",170,170,170
                cabbageSet         1,"AttackLine","colour",170,170,170
 elseif kOffTrig==1 then
                cabbageSet         1,"AttackLabel","fontColour",30,30,30
                cabbageSet         1,"AttackLine","colour",30,30,30
 endif
...
instr    99
 ...
 if changed(kpreset)==1 then
  ...
  if ipreset==1 then    ; xylobar
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.01,128,1           ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,1              ; modulation rate envelope
   iPreset[]      fillarray        2, 5.5, 0, 5.5, 0.4, 5, 0.5, 0, 0, 0, 0.8, 0.8, 0, 0.1, 0, 1, 0, 0.1, 0
  ...
  elseif ipreset==2 then    ; Scan Envelope
   ...
   iPreset[]      fillarray        1, 7.4, 0, 0.32, 0.006, 2, 0.5, 0, 0, 0, 0.8, 0.2, 1, 0.033, 1, 0.3, 0, 0, 0
  ...
  elseif ipreset==3 then    ; Soft Sustain
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.5,128,1              ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,1                ; modulation rate envelope
   iPreset[]      fillarray        3, 7.4, 1, 2.5, 0, 5, 0.5, 1, 1, 0.4, 0.8, 0.218, 0.5, 0.113, 0, 0.3, 0.2, 0.4, 0.5
  ...
  elseif ipreset==4 then    ; Rhodes
   ...
   giModEnv       ftgen            5,0,512,-5,1,512,1                ; modulation rate envelope
   iPreset[]      fillarray        3, 11.6, 0, 0.08, 0.038, 5, 0.98, 0, 0, 0, 0.8, 0.4, 0, 0.014, 1, 0.3, 0, 0.1, 0.42
  ...
  elseif ipreset==5 then    ; Good Vibes
   ...
   iPreset[]      fillarray        3, 16.6, 0, 0.3, 0.038, 5, 0.54, 0.005, 1, 0, 0.8, 0.323, 0.548, 0.014, 1, 8.84, 0, 0.1, 0.42
  ...
  elseif ipreset==6 then    ; Bad Vibes
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.5,128,1            ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,0.8            ; modulation rate envelope
   iPreset[]      fillarray        4, 16.6, 1, 0.08, 0.038, 5, 0.54, 0.012, 1, 0.21, 0.65, 0.058, 1, 0.007, 1, 8.84, 0.272, 1.313, 0.420
  ...
  elseif ipreset==7 then    ; Hard Beater
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.01,128,1           ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,1              ; modulation rate envelope
   iPreset[]      fillarray        1, 3, 0, 3, 0.365, 4, 0.47, 0, 0, 0, 0.25, 1.8, 0, 0.01, 0, 0.1, 0, 0.01, 0
  ...
  elseif ipreset==8 then    ; Stacking Plates
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.01,128,1         ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,1            ; modulation rate envelope
   iPreset[]      fillarray        4, 0.11, 0, 0.11, 0.269, 0, 0.26, 0, 0, 0, 0.8, 2.5, 0, 0.01, 0, 0.1, 0, 0.01, 0
  ...
  elseif ipreset==9 then    ; Bowed Plank
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.01,128,1         ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,1            ; modulation rate envelope
   iPreset[]      fillarray        2, 6, 0, 6, 0.144, 5, 0.47, 0.19, 1, 0, 0.8, 2.1, 0, 0.01, 0, 0.1, 0, 0.01, 0
  ...
  elseif ipreset==10 then    ; Interference
   ...
   iPreset[]      fillarray        1, 2.73, 0, 0.37, 0.011, 2, 0.29, 0, 0, 0, 0.8, 0.5, 1, 0.318, 1, 2.73, 0, 0.01, 0
  ...
  elseif ipreset==11 then    ; Ride Cymbal
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.1,128,1            ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,1              ; modulation rate envelope
   iPreset[]      fillarray        4, 18.12, 0, 0.28, 0.028, 3, 0.26, 0, 0, 0.25, 0.8, 0.3, 1, 1, 1, 10, 0, 0.01, 0
  ...
  elseif ipreset==12 then    ; Tubular Bell
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.1,128,1            ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,0.8            ; modulation rate envelope
   iPreset[]      fillarray        4, 30, 0, 30, 0.001, 1, 0.14, 0, 0, 0.25, 0.65, 0.5, 0.352, 0.178, 0, 10, 0, 0.01, 0.15
  ...
  elseif ipreset==13 then    ; Discord Bar
   ...
   giVelToAmp     ftgen            4,0,128,-5,0.1,128,1            ; map MIDI velocity to amplitude
   giModEnv       ftgen            5,0,512,-5,1,512,1              ; modulation rate envelope
   iPreset[]      fillarray        4, 2.95, 0, 0.28, 1.274, 1, 0.26, 0, 0, 0.25, 0.8, 0.437, 1, 0.425, 1, 10, 0, 0.01, 0
   
  endif
                  cabbageSet       1,"table2","tableNumber",2
                  cabbageSet       1,"table3","tableNumber",3
                  cabbageSet       1,"table4","tableNumber",4
                  cabbageSet       1,"table5","tableNumber",5
                  cabbageSetValue  "bc", iPreset[0]
                  cabbageSetValue  "T30", iPreset[1]
                  cabbageSetValue  "T30_Hold", iPreset[2]
                  cabbageSetValue  "rel", iPreset[3]
                  cabbageSetValue  "b", iPreset[4]
                  cabbageSetValue  "PolyLimit", iPreset[5]
                  cabbageSetValue  "wid", iPreset[6]
                  cabbageSetValue  "AttTim", iPreset[7]
                  cabbageSetValue  "AttAmt", iPreset[8]
                  cabbageSetValue  "RvbMix", iPreset[9]
                  cabbageSetValue  "RvbSize", iPreset[10]
                  cabbageSetValue  "OutAmp", iPreset[11]
                  cabbageSetValue  "ScanMix", iPreset[12]
                  cabbageSetValue  "scan", iPreset[13]
                  cabbageSetValue  "scan_x100", iPreset[14]
                  cabbageSetValue  "ModEnvDur", iPreset[15]
                  cabbageSetValue  "RndModAmt", iPreset[16]
                  cabbageSetValue  "RndModRte", iPreset[17]
                  cabbageSetValue  "PanDepth", iPreset[18]
