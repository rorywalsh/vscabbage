...
; PhISM Tambourine.csd
; Iain McCurdy, 2023

; PhISEM (Physically Informed Stochastic Event Modeling)

; Based on the opcode of Perry Cook's physical model of a tambourine, this example adds a number of performance enhancements 
...
; NUMBER            - number of timbrels
; DAMPING           - amount of energy loss upon each ricochet of a timbrel (high to low). This equates to the duration of the sound.
;                     setting this fully clockwise will produce a sustaining texture.
; FREQ              - fundamental resonant frequency of the tambourine
...
; PATTERN
; In this mode the tambourine is shaken repeatedly while a note is held
...
<Cabbage>
#define DIAL_PROPERTIES  colour("Grey"), trackerColour("silver"), fontColour("White"), textColour("White"), valueTextBox(1)

form caption("PhISEM Tambourine") size(1080,235), pluginId("RMSy"), colour( 45, 45, 45), guiMode("queue")

image bounds(5,5,1070,125), colour(0,0,0,0), outlineThickness(1)
{
rslider  bounds( 10, 25, 60,90), channel("Amp"), range(0,1,0.7,0.5), text("AMPLITUDE"), $DIAL_PROPERTIES
rslider  bounds( 80, 25, 60,90), channel("Num"), range(1,128,32,1,1), text("NUMBER"), $DIAL_PROPERTIES
rslider  bounds(150, 25, 60,90), channel("Damp"), range(0,0.75,0.65,0.5), text("DAMPING"), $DIAL_PROPERTIES
rslider  bounds(220, 25, 60,90), channel("Freq"), range(200,8000,2300,0.5,1), text("FREQ"), $DIAL_PROPERTIES
rslider  bounds(290, 25, 60,90), channel("Freq1"), range(200,8000,5600,0.5,1), text("FREQ.1"), $DIAL_PROPERTIES
rslider  bounds(360, 25, 60,90), channel("Freq2"), range(200,8000,8100,0.5,1), text("FREQ.2"), $DIAL_PROPERTIES
...
rslider  bounds(560, 25, 60,90), channel("AttTime"), range(0,12,0,0.5,0.01), text("ATT.TIME"), $DIAL_PROPERTIES
...
rslider  bounds(120,  0, 60,90), channel("PatternSpeed"), range(1,30,14.5,0.5,0.01), text("SPEED"), $DIAL_PROPERTIES
rslider  bounds(190,  0, 60,90), channel("Accent"), range(0,0.9,0.4), text("ACCENT"), $DIAL_PROPERTIES
}
...
label    bounds( 5,131,110,11), text("Iain McCurdy |2023|"), fontColour("silver"), align("left")
</Cabbage>
...
aL                 tambourine          iamp, idettack , inum, idamp, imaxshake, ifreq, ifreq1, ifreq2 ; amplitude (iamp) needs to be i-rate to be reliable
if iStMo==1 then
 aR                tambourine          iamp, idettack , inum, idamp, imaxshake, ifreq, ifreq1, ifreq2 ; amplitude (iamp) needs to be i-rate to be reliable
else
 aR                =                   aL
endif
...
if iPattern==0 then ; single shake
 aL,aR             Tambourine iNum+iBend, iVel, iBend
 ...
 ;                                                     p4   p5   p6       p7
                   schedkwhen kShake,0,0,2,0,1.5/kRate,iNum,kVel,kOutAmp, kBend
...
instr 99 ; monitor for CLIP indicator
 aL,aR             monitor
 if metro:k(1)==1 then
  kPeak            =                   0
 endif
