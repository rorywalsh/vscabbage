...
; Based on the opcode of Perry Cook's physical model of a bamboo shaker, this example adds a number of performance enhancements 
...
; NUMBER            - number of beans
; DAMPING           - amount of energy loss upon each ricochet of a bean (high to low). This equates to the duration of the sound.
...
; PATTERN
; In this mode the bamboo shaker is shaken repeatedly while a note is held
...
<Cabbage>
#define DIAL_PROPERTIES  colour("Tan"), trackerColour("silver"), fontColour("White"), textColour("White"), valueTextBox(1)

form caption("PhISEM Bamboo Shaker") size(1080,235), pluginId("Bamb"), colour(100,80,80), guiMode("queue")
...
rslider  bounds( 80, 25, 60,90), channel("Num"), range(1,128,2,1,1), text("NUMBER"), $DIAL_PROPERTIES
rslider  bounds(150, 25, 60,90), channel("Damp"), range(-0.3,0.05,0,4), text("DAMPING"), $DIAL_PROPERTIES
rslider  bounds(220, 25, 60,90), channel("Freq"), range(200,8000,2800,0.5,1), text("FREQ"), $DIAL_PROPERTIES
rslider  bounds(290, 25, 60,90), channel("Freq1"), range(200,8000,2240,0.5,1), text("FREQ.1"), $DIAL_PROPERTIES
rslider  bounds(360, 25, 60,90), channel("Freq2"), range(200,8000,3360,0.5,1), text("FREQ.2"), $DIAL_PROPERTIES
...
rslider  bounds(120,  0, 60,90), channel("PatternSpeed"), range(0.25,30,2,0.5,0.01), text("SPEED"), $DIAL_PROPERTIES
...
aL          bamboo              iamp, idettack , inum, idamp, imaxshake, ifreq, ifreq1, ifreq2 ; amplitude (iamp) needs to be i-rate to be reliable
if iStMo==1 then
 aR	        bamboo              iamp, idettack , inum, idamp, imaxshake, ifreq, ifreq1, ifreq2 ; amplitude (iamp) needs to be i-rate to be reliable
else
 aR         =                   aL
endif
...
if iPattern==0 then ; single shake
 aL,aR   Bamboo     iNum+iBend, iVel, iBend
...
instr 2 ; used in 'pattern' mode
aL,aR   Bamboo     p4,p5,p7
iRelTime cabbageGetValue "RelTime"  ; release time
aEnv    expsegr     1,iRelTime,0.001
aL      *=         aEnv*p6
aR      *=         aEnv*p6
        outs       aL,aR
endin
