...
; Mode Resonators.csd
; Written by Iain McCurdy, 2012. Updated: 2014, 2024
; 
...
; Excitation
; ----------
; Input        -    nature of the excitation:
;                     1. a rubbing movement
;                     2. a single strike
;                     3. live audio input (still need to press MIDI keys to activate)
;                     4. a periodic striking live a motorised ringer. Rate of this ringer can be adjusted.
; Movement     -    choose between a random rubbing movement and a circular one.
; Amp.(dB)     -    amplitude in decibels of the excitation signal
; HPF          -    the cutoff frequency of a high-pass filter applied to the excitation signal
; LPF          -    the cutoff frequency of a low-pass filter applied to the excitation signal. In series with the high-pass filter.
; Jit. Dep     -    Jitter Depth. Amplitude of the random variation of the nature of the rubbing excitation signal.
;                    larger values will produce less resonance like a brushing or blowing rather than a rubbing.
; Jit. Rate    -    Jitter Rate. Rate of the random variation of the nature of the rubbing excitation signal.
;                    larger values imply a faster rubbing/brushing
; Rate         -    (ringer mode only) rate of the 'ringer' impulses
...
;
; MIDI
; Pitch bend shifts the pitch by +/-2 semitones
; Modulation wheel also controls Q

<Cabbage>
form caption("Mode Resonators"), size(790,370), pluginId("RubR"), guiMode("queue")
image                       bounds(0, 0,790,370), colour(20,20,30), shape("sharp"), oulinecolour("brown"), outlineThickness(4)

#define RSLIDER_DESIGN1  colour(200,200,110,200), trackerColour(200,200,110), valueTextBox(1)
#define RSLIDER_DESIGN2  colour(150,150,180,200), trackerColour(150,150,180), valueTextBox(1)
#define RSLIDER_DESIGN3  colour(180,150,150,200), trackerColour(180,150,150), valueTextBox(1)
#define RSLIDER_DESIGN4  colour(150,180,180,200), trackerColour(150,180,180), valueTextBox(1)
#define RSLIDER_DESIGN5  colour(100,130,180,200), trackerColour(100,130,180), valueTextBox(1)
#define RSLIDER_DESIGN6  colour(180,150,150,200), trackerColour(180,150,150), valueTextBox(1)


;EXCITATION
groupbox bounds( 5,  5, 430,125), text("Excitation"), channel("excitation")
{
label    bounds(  5, 27,80,13), text("Input")
combobox bounds(  5, 40,80,25), text("Rubbing","Strike","Live","Ringer"), channel("Input"), value(1)
label    bounds(  5, 75,80,13), text("Movement"), channel("MovementLabel")
combobox bounds(  5, 88,80,25), text("Random","Circular"), channel("Movement"), value(1)
rslider  bounds( 90, 25,70,90), text("Amp.(dB)"), channel("ImpDB"), range(-70,60, 0) $RSLIDER_DESIGN1
rslider  bounds(155, 25,70,90), text("HPF"), channel("HPF"), range(20, 20000, 20, 0.5, 1) $RSLIDER_DESIGN1
rslider  bounds(220, 25,70,90), text("LPF"), channel("LPF"), range(20, 20000, 20000, 0.5, 1) $RSLIDER_DESIGN1
rslider  bounds(285, 25,70,90), text("Jit. Dep."), channel("JitDep"), range(0, 5.00, 0.4,0.5) $RSLIDER_DESIGN1
rslider  bounds(350, 25,70,90), text("Jit. Rate"), channel("JitRte"), range(0.01,100, 3,0.5) $RSLIDER_DESIGN1
rslider  bounds(350, 25,70,90), text("Rate"), channel("ringerRate"), range(4, 20, 12, 0.5) $RSLIDER_DESIGN1
}

;RESONATORS
groupbox bounds(440,  5,345,125), text("Resonators"), channel("resonators")
{
label    bounds( 15, 27,160, 12), text("Instrument")
combobox bounds( 15, 40,160, 25), channel("sound"), value(21), text("Single", "Dahina", "Banyan", "Xylophone", "Tibetan Bowl 180mm", "Spinel Sphere", "Pot Lid", "Red Cedar Wood Plate", "Tubular Bell", "Redwood Wood Plate", "Douglas Fir Wood Plate", "Uniform Wooden Bar", "Uniform Aluminium Bar", "Vibraphone 1", "Vibraphone 2", "Chladni Plates", "Tibetan Bowl 152mm", "Tibetan Bowl 140mm", "Wine Glass", "Small Handbell", "Albert Clock Bell", "Wood Block","Harmonic 10","Harmonic 20","Harmonic 30","Harmonic Odd 10","Harmonic Odd 20","Anvil")
label    bounds( 11, 75,130, 13), text("Number of Partials:"), align("right")
nslider  bounds(146, 72, 29, 20), channel("npartials"), range(1, 10000, 22, 1, 1), active(0)
rslider  bounds(190, 25, 70, 90), text("Q"), channel("Q"), range(1, 10000, 2000, 0.5, 1), $RSLIDER_DESIGN2
rslider  bounds(255, 25, 70, 90), text("Bright"), channel("bright"), range(-4.00, 4, 0), $RSLIDER_DESIGN2
}

;TUNING                                        
groupbox bounds(  5,135,220,125), text("Tuning"), channel("tuning")
{
rslider  bounds( 10, 25, 70, 90), text("Detune"), channel("detune"), range(0, 5, 0), $RSLIDER_DESIGN3
rslider  bounds( 75, 25, 70, 90), text("Shift"), channel("shift"), range(0, 127, 0), $RSLIDER_DESIGN3
rslider  bounds(140, 25, 70, 90), text("Compress"), channel("compress"), range(0, 1, 0), $RSLIDER_DESIGN3
}

;POLYPHONY                                        
groupbox bounds(230,135,245,125), text("Polyphony"), channel("polyphony")
{
button   bounds(  8, 25, 84, 25), text("Polyphonic","Monophonic"), channel("monopoly"), value(0)
hslider  bounds(  5, 50, 95, 30), colour("chocolate"), channel("GlissTime"), range(0.005,20, 0.1, 0.5, 0.0001), visible(0)
label    bounds(  5, 74, 95, 12), text("Gliss Time"), channel("GlissTimeLabel"), visible(0)
rslider  bounds(100, 25, 70, 90), text("Poly.Limit"), channel("PolyLimit"), range(0, 20, 5,1,1), $RSLIDER_DESIGN4
rslider  bounds(165, 25, 70, 90), text("Rel.Time"), channel("RelTim"), range(0.01, 5,0.2,0.5,0.01), $RSLIDER_DESIGN4
}

;CHORUS
groupbox bounds(480,135,220,125), text("Chorus"), channel("chorus")
{
rslider  bounds( 10, 25, 70, 90), text("Mix"), channel("ChoMix"), range(0, 1.00, 0.5), $RSLIDER_DESIGN5
rslider  bounds( 75, 25, 70, 90), text("Depth"), channel("ChoDep"), range(0, 0.1, 0.01,0.5, 0.001), $RSLIDER_DESIGN5
rslider  bounds(140, 25, 70, 90), text("Rate"), channel("ChoRte"), range(0, 20, 0.96, 0.5), $RSLIDER_DESIGN5
}                                                            

;OUTPUT
groupbox bounds(705,135, 80,125), text("Output"), channel("output")
{
rslider  bounds( 10, 25, 70, 90), text("Level"), channel("OutLev"), range(0, 1.00, 0.25), $RSLIDER_DESIGN6
}

keyboard bounds( 5, 265, 780, 85)

label    bounds( 5,352,120, 13), text("Iain McCurdy |2012|"), fontColour("silver"), align("left")
...
instr    1    ;read in widgets (always on)
    ...
    if gkModWhlT==1 then
    gkModWhl     =                     (gkModWhl/127) ^ 3
                 cabbageSetValue       "Q", (gkModWhl*9999)+1
    endif
    gkInput      cabbageGetValue       "Input"               ; 1.Rubbing 2.Strike 3.Live 4.Ringer
    if changed:k(gkInput)==1 then
     if gkInput==1 then ; bowing
      cabbageSet k(1), "Movement", "visible", 1
      cabbageSet k(1), "MovementLabel", "visible", 1
      cabbageSet k(1), "JitDep", "visible", 1
      cabbageSet k(1), "JitRte", "visible", 1
      cabbageSet k(1), "ringerRate", "visible", 0
     elseif gkInput==2 then ; strike
      cabbageSet k(1), "Movement", "visible", 0
      cabbageSet k(1), "MovementLabel", "visible", 0
      cabbageSet k(1), "JitDep", "visible", 0
      cabbageSet k(1), "JitRte", "visible", 0
      cabbageSet k(1), "ringerRate", "visible", 0
     elseif gkInput==3 then ; live
      ...
      cabbageSet k(1), "JitRte", "visible", 0
      cabbageSet k(1), "ringerRate", "visible", 0
     elseif gkInput==4 then ; ringer
      cabbageSet k(1), "Movement", "visible", 0
      cabbageSet k(1), "MovementLabel", "visible", 0
      cabbageSet k(1), "JitDep", "visible", 0
      cabbageSet k(1), "JitRte", "visible", 0
      cabbageSet k(1), "ringerRate", "visible", 1
     endif
    endif
    gkMovement   cabbageGetValue       "Movement"
    gkQ          cabbageGetValue       "Q"                   ; read in widgets
    kImpDB       cabbageGetValue       "ImpDB"
    gkHPF        cabbageGetValue       "HPF"
    gkLPF        cabbageGetValue       "LPF"
    gkJitDep     cabbageGetValue       "JitDep"
    gkJitRte     cabbageGetValue       "JitRte"
    gkringerRate cabbageGetValue       "ringerRate"
    gkbright     cabbageGetValue       "bright"
    gkdetune     cabbageGetValue       "detune"
    gkdetune     portk                 gkdetune,0.1
    gkshift      cabbageGetValue       "shift"
    gkshift      portk                 gkshift,0.1
    gkcompress   cabbageGetValue       "compress"    
    gkcompress   portk                 gkcompress,0.1
    gkmonopoly   cabbageGetValue       "monopoly"
                 cabbageSet            changed:k(gkmonopoly), "GlissTime", "visible", gkmonopoly
                 cabbageSet            changed:k(gkmonopoly), "GlissTimeLabel", "visible", gkmonopoly
                 cabbageSet            changed:k(gkmonopoly), "PolyLimit", "visible", 1-gkmonopoly
    
    gkGlissTime  cabbageGetValue       "GlissTime"
    gksound      cabbageGetValue       "sound"
    gkChoMix     cabbageGetValue       "ChoMix"
    gkChoRte     cabbageGetValue       "ChoRte"
    gkChoDep     cabbageGetValue       "ChoDep"
    gkOutLev     cabbageGetValue       "OutLev"
...
instr    3                                                ; impulse and modal resonators instrument
    ...
    /* POLYPHONY CONTROL */
    iPolyLimit   cabbageGetValue       "PolyLimit"
    ...
    /* monophonic */     kNoteTrig  changed      gknum                         ; ...GENERATE A TRIGGER IS A NEW NOTE NUMBER IS GENERATED (FROM INSTR. 1)
     gkNoteTrig  =                     0
    ...
    knum2        =                     (knum * (1-gkcompress)) + gkshift + gkPB     ; shifted and compressed version  of knum
    ...
    elseif gkInput==2 then                                 ; 2. strike impulse
     itim        scale_i               ivel^0.125,0.001,0.03
     aInput      tablei                line:a(0,itim,1),gihanning,1
     aInput      *=                    ivel*gkImpAmp
     aInput      buthp                 aInput,gkHPF
     aInput      butlp                 aInput,gkLPF     
    elseif gkInput==3 then                                 ; 3. live
     a1,a2       ins
     aInput      =                     (a1+a2)*kactivePort*gkImpAmp          
     aInput      buthp                 aInput,gkHPF
     aInput      butlp                 aInput,gkLPF     
    else                                                   ; 4. ringer
     aInput      gausstrig             1, gkringerRate, 0.1
     aInput      *=                    linsegr:k(1,0.3,0) * kactivePort * gkImpAmp 
     aInput      buthp                 aInput,gkHPF
     aInput      butlp                 aInput,gkLPF     
    endif
...
instr    5    ;update 'Number of Partials' number box
    ...
    knvoices     init                 invoices                        ; create krate version of invoices
                 cabbageSetValue      "npartials",knvoices            ; send value to number box
endin
