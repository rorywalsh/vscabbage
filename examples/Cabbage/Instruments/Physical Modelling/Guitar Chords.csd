...
label bounds(5,252,100,11), text("Iain McCurdy |2013|"), fontColour("silver"), align("left")
</Cabbage>
...
instr    2    ; string pluck
 ...
 iGuitarType     chnget          "GuitarType"
  ...
  if p5<5 then                                                ; strings 1 - 4 are in octaves in a 12-string guitar
   a1            wgpluck2        iplk, iamp/2, p4,          0.2+kmvt1, kdamp * giSRcomp
   a2            wgpluck2        iplk, iamp/2, p4*(2+idtn), 0.2+kmvt2, kdamp * giSRcomp
  else                                                        ; strings 5 & 6 are in unison in a 12-string guitar
   a1            wgpluck2        iplk, iamp/2, p4,          0.2         +kmvt1, kdamp * giSRcomp
   a2            wgpluck2        iplk, iamp/2, p4*(1+idtn), 0.2+rnd(0.5)+kmvt2, kdamp * giSRcomp
  endif
   a2            delay           a2,0.01+rnd(0.01)                ; slight delay to second string
  asig           =               a1+a2
 else                                                         ; otherwise 6 string
  kmvt           jspline         0.15,0.2,0.4        
  asig           wgpluck2        iplk, iamp, p4, 0.2+kmvt, kdamp * giSRcomp
 endif
