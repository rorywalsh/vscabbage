...
; Glockenspiel (physical model using modal synthesis)
; Iain McCurdy, 2022

; Includes a note damping mechanism which is applied when a key is held (as if the beater is held against the bar).
...
; CONTROLS
 ...
 ;                 at zero, this effect is bypassed
 
 ; B A R S
 ; Sustain      - sustain time of the resonance. For this to be most apparent, Hold Dampengin should be at zero.
 ; Release      - dampening that is applied when a note is released. At its maximum, no dampening is applied upon note release.
 ; Level        - output level.

<Cabbage>
form caption("Glockenspiel") size(485, 242), colour(20,20,30), pluginId("Glck"), guiMode("queue")
...
rslider bounds(  5, 30, 90, 90) range(0, 1,  1, 1, 0.001), channel("MHard"), text("Hardness"), $RSliderStyle  
rslider bounds( 95, 30, 90, 90) range(0, 1,0.5, 1, 0.001), channel("MDamp"), text("Hold Damping"), $RSliderStyle
}
...
rslider bounds(  5, 30, 90, 90) range(-8, 8, 0, 1, 0.001), channel("BSus"), text("Sustain"), $RSliderStyle
...
keyboard bounds(  5,135,475, 95), keypressBaseOctave(8),  whiteNoteColour("Silver"), blackNoteColour("Silver"), keyWidth(20), value(72),  mouseOverKeyColour(80,80,80),

label bounds(3,231,100,11), text("Iain McCurdy |2022|"), fontColour("grey"), align("left")
</Cabbage>
<CsoundSynthesizer>
...
giNotesDamp          ftgen  0,0,128,2,0             ; note-by-note damp
...
iRandOS   =                 0.01                                                                                        ; random offset ratio of partials for timbral variation
...
kEnv      expon            1, 0.001 + ((1-i(kMHard))*0.1), 0.001  ; a quickly decaying amplitude envelope for the impulse sound
...
if release:k()==0 then                              ; while a note is being held...
 ; HOLDING A STRIKE WILL CAUSE A DAMPED HIT while note is being held
 if iMDamp>0 then
  kQ       =               1
           tablew          expseg:k(1,0.05,1,1.01-iMDamp,0.001),iNote,giNotesDamp
 else
  kQ       =               1
 endif
...
else                                                ; when in release stage...
 kQ       =                octave(kBRel)            ; scale according to bar release setting
 iRel     limit            10*1567.982/iCPS, 1, 30  ; amplitude release time
 aRel     linsegr          1, iRel, 0
endif

; hold damping. Using a table and a value associated with each note means that resonating notes of the same pitch in their release stage will also be affected by mallet hold damping.
 if iMDamp>0 then
 kQ       *=               table:k(iNote,giNotesDamp)
 endif
 
; smooth changes made to Q
