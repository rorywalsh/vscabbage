...
; Most controls are adjusted using number boxes. 
...
; Trig.Angle   - the angle of the line over which pendulum bobs will trigger note events
...
<Cabbage>
form size(300,555), guiRefresh(16), caption("Pendulum Waves"), colour("white"), pluginId("PeWa")
...
vslider bounds( 25,100,20,60), channel("Amp1"), range(0,1,1,0.5)
vslider bounds( 45,100,20,60), channel("Amp2"), range(0,1,1,0.5)
vslider bounds( 65,100,20,60), channel("Amp3"), range(0,1,1,0.5)
vslider bounds( 85,100,20,60), channel("Amp4"), range(0,1,1,0.5)
vslider bounds(105,100,20,60), channel("Amp5"), range(0,1,1,0.5)
vslider bounds(125,100,20,60), channel("Amp6"), range(0,1,1,0.5)
vslider bounds(145,100,20,60), channel("Amp7"), range(0,1,1,0.5)
vslider bounds(165,100,20,60), channel("Amp8"), range(0,1,1,0.5)
vslider bounds(185,100,20,60), channel("Amp9"), range(0,1,1,0.5)
vslider bounds(205,100,20,60), channel("Amp10"), range(0,1,1,0.5)
vslider bounds(225,100,20,60), channel("Amp11"), range(0,1,1,0.5)
vslider bounds(245,100,20,60), channel("Amp12"), range(0,1,1,0.5)
...
label   bounds(  5,542, 110, 12), text("Iain McCurdy |2020|"), fontColour("darkGrey"), align("left")
...
gSfilepath         init                ""
gig                =                   9.81     ; acceleration due to gravity
...
if changed:k(kRecOut)==1 then
 ...
 gkL12             =                   ((60/(2*$M_PI*gkFreqs[11]))^2)*gig ; length of the last pendulum
 iCount            =                   0
 loop:
 ...
 ; set amplitudes
                   tablew              chnget:k("Amp1"), 0, giAmps
                   tablew              chnget:k("Amp2"), 1, giAmps
                   tablew              chnget:k("Amp3"), 2, giAmps
                   tablew              chnget:k("Amp4"), 3, giAmps
                   tablew              chnget:k("Amp5"), 4  ,giAmps
                   tablew              chnget:k("Amp6"), 5, giAmps
                   tablew              chnget:k("Amp7"), 6, giAmps
                   tablew              chnget:k("Amp8"), 7, giAmps
                   tablew              chnget:k("Amp9"), 8, giAmps
                   tablew              chnget:k("Amp10"),9, giAmps
                   tablew              chnget:k("Amp11"), 10, giAmps
                   tablew              chnget:k("Amp12"), 11, giAmps
...
instr   2   ; a pendulum
 ...
 iRad              =                   14            ; radius of bob (visual only)
 ...
 kL                =                   ((60/(2*$M_PI*kFreq))^2)*gig           ; calculate length of pendulum  
 if ksync==0 then                         ; is sync button is off (follow normal behaviour)
  kPeriod          =                   2 * $M_PI * sqrt(kL/gig) * (1/kSpeed)  ; period of a pendulum swing - dependent upon pendulum length but not mass
 else
  kPeriod          =                   2 * $M_PI * sqrt(gkL12/gig) * (1/kSpeed)   ; sync is on: all pendulums use the same length value
 endif
