...
<Cabbage>
form caption("Bouncing Ball Ensemble") size(610, 490), pluginId("bnce"), colour("Black"), guiMode("queue")

; box 1 
xypad    bounds(0, 0, 230, 250), colour(30,30,30), channel("x1", "y1"), ballColour(255,100,100), fontColour(0,0,0,0), rangeX(0, 1, 0) rangeY(0, 1, 0)

; box 2
xypad    bounds(230,  0,270,250), colour(30,30,30), channel("x2", "y2"), ballColour(255,255,100), fontColour(0,0,0,0), rangeX(0, 1.00, 0), rangeY(0, 1, 0)

; box 3
xypad    bounds(  0,250,500,240), colour(30,30,30), channel("x3", "y3"), ballColour(100,100,255), fontColour(0,0,0,0), rangeX(0, 1.00, 0), rangeY(0, 1, 0)

; box 4
xypad    bounds(500,  0,110,490), colour(30,30,30), channel("x4", "y4"), ballColour(100,255,100), fontColour(0,0,0,0), rangeX(0, 1.00, 0), rangeY(0, 1, 0)
, ballColour(255,100,100)
label bounds(  5,477,110, 12), text("Iain McCurdy |2014|"), align("left"), fontColour("tan")
...
instr    1
 ; box 1
 kx                  cabbageGetValue    "x1"                        ; read in x and y position for this box
 ky                  cabbageGetValue    "y1"
 ...
 ; box 2
 kx            cabbageGetValue         "x2"                            ; read in x and y position for this box
 ky            cabbageGetValue         "y2"
 ktrig1,ktrig2,ktrig3,ktrig4    XYPadEdgeTriggers    kx,ky    ; call UDO that senses barrier collisions    
               schedkwhen     (ktrig1+ktrig2+ktrig3+ktrig4)*kClickSum,0,0,102,0,1.5,ky,kx                  ; trigger instruments

 ; box 3
 kx            cabbageGetValue         "x3"                                                          ; read in x and y position for this box
 ky            cabbageGetValue         "y3"
 ktrig1,ktrig2,ktrig3,ktrig4    XYPadEdgeTriggers    kx,ky                                  ; call UDO that senses barrier collisions
               schedkwhen     (ktrig1+ktrig2+ktrig3+ktrig4)*kClickSum,0,0,103,0,0.1,ky,kx   ; trigger instruments 

 ; box 4
 kx    cabbageGetValue    "x4"                        ; read in x and y position for this box
 ky    cabbageGetValue    "y4"
 ktrig1,ktrig2,ktrig3,ktrig4    XYPadEdgeTriggers    kx,ky    ; call UDO that senses barrier collisions
...
instr    101    ; longer harmonic pluck sound
 ; flash box on impact
 kColour       linseg         255,0.1,30
               cabbageSet     changed:k(kColour), "x1", "colour", kColour, kColour, kColour
 inum          =              48 + int(24 * p4)
...
instr    102    ; longer inharmonic pluck sound
 ; flash box on impact
 kColour       linseg         255,0.1,30
               cabbageSet     changed:k(kColour), "x2", "colour", kColour, kColour, kColour
 inum          =              60 + int(24 * p4)
 aEnv          linsegr        0, 0.005, 1, p3-0.105, 1, 0.1, 0 ; amplitude envelope
 iPlk          random         0.1, 0.3                         ; point at which to pluck the string
 iDtn          random         -0.05, 0.05                      ; random detune
...
instr    103    ; short sine impulse - no dry signal, only reverb
 ; flash box on impact
 kColour       linseg         255,0.1,30
               cabbageSet     changed:k(kColour), "x3", "colour", kColour, kColour, kColour
 inum          =              84 + int(18 * p4)
 aEnv          linsegr        0, 0.005, 1, p3-0.105, 1, 0.1, 0 ; amplitude envelope
...
instr    104    ; short harmonic pluck sound - wide picth range
 ; flash box on impact
 kColour       linseg         255,0.1,30
               cabbageSet     changed:k(kColour), "x4", "colour", kColour, kColour, kColour
 inum          =              48 + int(48 * p4)
 aEnv          linsegr        0, 0.005, 1, p3-0.105, 1, 0.1, 0 ; amplitude envelope
 iPlk          random         0.1, 0.3                         ; point at which to pluck the string
 iDtn          random         -0.05, 0.05                      ; random detune
