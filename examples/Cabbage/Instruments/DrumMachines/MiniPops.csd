...
; a rather enigmatic way. Unfortunately, I haven't quite figured out how this worked yet so in my implementation they simply mix.

; The user can choose between 'STANDALONE' and 'HOST' modes. In 'HOST' mode the tempo is taken from a DAW host 
;  and the On/Off button is controlled from the DAW's transport.

<Cabbage>
form size(840,250), caption("Mini Pops"), pluginId("MPop"), colour(255,255,205) ;, guiMode("queue")
...
checkbox bounds( 15, 16, 12, 13), channel("r1") fontColour("white") colour("yellow"), value(1), radioGroup(1)
checkbox bounds( 15, 28, 12, 13), channel("r2") fontColour("white") colour("yellow"), radioGroup(1)
...
image    bounds(140, 97, 70, 25), colour("Grey"), corners(6)
checkbox bounds(143,100, 64, 19), channel("OnOff"), fontColour("black"), colour:1("yellow"),value(1)
label    bounds(140,127, 70, 11), text("Start/Stop"),     fontColour("black"), align("centre")

button   bounds(140, 77, 70, 15), text("STANDALONE","HOST"), channel("ClockSource"), value(0), fontColour:0("yellow"), fontColour:1("yellow")
...
;csoundoutput bounds(5, 255, 500, 90), text("string"), colour("colour"), channel("chan")
...
kClockSource chnget    "ClockSource"            ;
 if kClockSource==0 then                        ; if internal clock source has been chosen...
    ...
    gktempo        chnget    "HOST_BPM"         ; tempo taken from host BPM
                   chnset    chnget:k("IS_PLAYING"), "OnOff"
endif
;printk2 gkOnOff
    ; volume controls
...
#define    SOLO_MUTE_STATUS(N)    
    ...
    kr1           chnget      "r1" ; row 1, 2
    kr2           chnget      "r2"
    gkUpperLower  =           kr1 == 1 ? 1 : 2
...
instr    2    ;READ MIDI AND TRIGGER NOTES
    ...
    if iinstr==11 then                         ;if this is instr 11 (guiro). i.e. sustains and responds to midi note release...
     ...
     if krel==1 then                           ;if this note is released...
      turnoff2    101+iinstr,0,1               ;...turn off the subinstrument
     endif
