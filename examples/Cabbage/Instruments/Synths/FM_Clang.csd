...
rslider  bounds(  0, 25, 7z5, 75), text("Level"),    channel("Amp"),    range(0,8.00,0.5,0.5),        valueTextBox(1), textBox(1), $RSliderStyle
...
label    bounds( 62, 95,110, 12), text("Iain McCurdy |2014|"), fontColour("silver"), align("right")
}
...
instr    2    ; Sound producing instrument. Triggered by MIDI notes.
 ...
 asig         foscil       iAVel, icps*cent(-gkDetune*idetune)*semitone(gkPB), icar     +aNse, gkmod, kIEnv, gisine    ; FM pair
 aModSig      foscil       iAVel, icps*cent(-gkDetune*idetune)*semitone(gkPB), icar+amod+aNse, gkmod, kIEnv, gisine    ; FM pair with modulation
 ...
 if gkDtnMix>0&&gkDetune>0 then
  ...
  asig2       foscil       iAVel, icps*cent(gkDetune*idetune)*semitone(gkPB), icar     +aNse, gkmod, kIEnv, gisine    ; Detuning applied to fundemental
  aModSig2    foscil       iAVel, icps*cent(gkDetune*idetune)*semitone(gkPB), icar+amod+aNse, gkmod, kIEnv, gisine
