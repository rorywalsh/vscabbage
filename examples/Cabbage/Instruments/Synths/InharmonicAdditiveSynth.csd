...
; Random
; ------
; Panning -    note-by-note random panning location
; Tuning  -    note-by-note random tuning
; Tone    -    random tone variations each time a note is played (tuning and phase cancellations)
...
; Ring Modulation
; ---------------
; On/Off  -    Ring modulator on/off
; Freq    -    Frequency of the modulating sine wave

<Cabbage>
form caption("Inharmonic Synth"), size(760, 335), pluginId("InSy"), colour("black"), guiMode("queue")

#define RSLIDER_DESIGN1 colour(255,150,150), trackerColour( 55,  0,  0), valueTextBox(1)
#define RSLIDER_DESIGN2 colour("cornflowerblue"), trackerColour(0,0,100), valueTextBox(1)
#define RSLIDER_DESIGN3 colour("white"), trackerColour("silver"), valueTextBox(1)
#define RSLIDER_DESIGN4 colour(255,255,200), trackerColour(55,55,0), valueTextBox(1)
#define RSLIDER_DESIGN5 colour(200,255,255), trackerColour( 0,55,55), valueTextBox(1)
#define RSLIDER_DESIGN6 colour(255,200,255), trackerColour(55, 0,55), valueTextBox(1)

groupbox bounds(10, 10,260,110), text("Amplitude Envelope")
{
rslider bounds(  0, 25, 60, 80), text("Att."),  channel("AAtt"), range(0.0001,1.00,0.0001,0.25,0.0001), $RSLIDER_DESIGN1
rslider bounds( 50, 25, 60, 80), text("Dec."),  channel("ADec"), range(0.001,30.00,7,0.5), $RSLIDER_DESIGN1
rslider bounds(100, 25, 60, 80), text("Sus."),  channel("ASus"), range(0, 1.00, 0), $RSLIDER_DESIGN1
rslider bounds(150, 25, 60, 80), text("Rel."),  channel("ARel"), range(0.01, 30.00, 7,0.5), $RSLIDER_DESIGN1

rslider bounds(200, 25, 60, 80), text("Level"), channel("Lev"), range(0, 5.00, 0.1,0.5), $RSLIDER_DESIGN1
}

groupbox bounds(275,10,160,110), text("Instrument")
{
...
groupbox bounds(440,10,160,110), text("Random")
{
rslider bounds(  0, 25, 60, 80), text("Panning"),  channel("Panning"), range(0, 1, 0), $RSLIDER_DESIGN4
rslider bounds( 50, 25, 60, 80), text("Tuning"),  channel("Tuning"), range(0, 1, 0), $RSLIDER_DESIGN4
rslider bounds(100, 25, 60, 80), text("Tone"),  channel("ToneVar"), range(0, 1, 0), $RSLIDER_DESIGN4
}

groupbox bounds(610, 10,140,110), text("Ring Modulation")
{
checkbox bounds( 10, 30, 60, 15), channel("RMOnOff") text("On/Off")
rslider  bounds( 70, 25,60,80), text("Freq."), channel("RMFreq"), range(-5000, 5000,-350,1,1), $RSLIDER_DESIGN5
}

groupbox bounds(10,125,260,110), text("Filter Envelope")
{
rslider bounds(  0, 25, 60, 80), text("Amount"),  channel("FAmt"), range(0, 10.0, 8), $RSLIDER_DESIGN2
rslider bounds( 50, 25, 60, 80), text("Dec."),   channel("FDec"), range(0.001,30.00,7,0.5), $RSLIDER_DESIGN2
rslider bounds(100, 25, 60, 80), text("Sus."),   channel("FSus"), range(0,1.00,0), $RSLIDER_DESIGN2
rslider bounds(150, 25, 60, 80), text("Rel."),   channel("FRel"), range(0.1,30.00,7,0.5), $RSLIDER_DESIGN2
rslider bounds(200, 25, 60, 80), text("Offset"), channel("F_OS"), range(-4,10.0, 0), $RSLIDER_DESIGN2
}

groupbox bounds(275,125, 160,110), text("Chorus")
{
rslider  bounds(  0, 25,60,80), text("Mix"),   channel("ChoMix"), range(0, 1.00, 0.5), $RSLIDER_DESIGN3
rslider  bounds( 50, 25,60,80), text("Depth"), channel("ChoDep"), range(0, 0.1, 0.01,0.5, 0.001), $RSLIDER_DESIGN3
rslider  bounds(100, 25,60,80), text("Rate"),  channel("ChoRte"), range(0, 20.00, 0.96, 0.5), $RSLIDER_DESIGN3
}

groupbox bounds(440,125,310,110), text("Reverb")
{
checkbox bounds( 15, 30, 60, 15), channel("RvbOnOff") text("On/Off"), value(1)
rslider  bounds( 90, 25,60,80), text("Mix."), channel("RvbMix"), range(0, 1, 0.3), $RSLIDER_DESIGN6
rslider  bounds(150, 25,60,80), text("Size"), channel("RvbSize"), range(0.4, 1, 0.8, 2), $RSLIDER_DESIGN6
rslider  bounds(210, 25,60,80), text("Damping"), channel("RvbDamp"), range(200, 18000, 8000, 0.5, 1), $RSLIDER_DESIGN6
}


keyboard bounds( 10,240,740, 80)

label    bounds( 10,321,110, 12), text("Iain McCurdy |2012|"), fontColour("silver"), align("left")
</Cabbage>
...
giPanning   ftgen    0, 0, 128, 21, 6, 1
giTuning    ftgen    0, 0, 128, 21, 6, 1
...
instr    1    ; Read in widgets
    ...
    if trigger:k(gkRvbOnOff,0.5,0)==1 then
     event "i",4,0,36000 ; reverb
    endif
    gkInstr   cabbageGetValue    "Instr"
    ...
    gkFDec    cabbageGetValue    "FDec"
    gkFSus    cabbageGetValue    "FSus"
    gkFRel    cabbageGetValue    "FRel"
    gkFAmt    cabbageGetValue    "FAmt"
    gkF_OS    cabbageGetValue    "F_OS"
endin
...
instr    2    ;SOUND GENERATING INSTRUMENT
    ...
    ; random panning and tuning
    iPanVal   ntrpol    0.5, 0.5 + (table:i(inum,giPanning)*0.5), cabbageGetValue:i("Panning")
    iTuneVal  ntrpol    0, table:i(inum,giTuning) * 100, cabbageGetValue:i("Tuning")
    icps      *=        cent(iTuneVal)
    
    icps      *=        cent(bexprnd:i(cabbageGetValue:i("ToneVar")*2))
    ...
    asig      *=        poscil:a(1,expon:a(1,0.001,0.001)*5000)
    asig      +=        delay:a(asig,0.5/icps) * random:i(0,cabbageGetValue:i("ToneVar"))
    if cabbageGetValue:k("RMOnOff")==1 then
     asig      *=        poscil:a(1,a(cabbageGetValue:k("RMFreq")))
    endif
    ...
    kcf_oct   limit     octcps(icps*0.25) + gkF_OS + (gkFAmt*kcf_env*ivel^2), 4, 14
    ...
    asig      butlp     asig*ivel, cpsoct(kcf_oct)                                      ; LOW PASS FILTER THE SOUND (SUBTRACTIVE SYNTHESIS)
    aL,aR     pan2      asig, iPanVal
    gasendL   =         gasendL + aL
    gasendR   =         gasendR + aR
...
instr    3    ; Chorus Effect
    ...
    gasendL   +=         achoL*gkChoMix
    gasendR   +=         achoR*gkChoMix
    
endin


instr    4 ; reverb
    if gkRvbOnOff==0 then
     turnoff
    endif
    kRvbMix  cabbageGetValue "RvbMix"
    kRvbSize cabbageGetValue "RvbSize"
    kRvbDamp cabbageGetValue "RvbDamp"
    aRvbL,aRvbR reverbsc gasendL*kRvbMix,gasendR*kRvbMix,kRvbSize,kRvbDamp
    gasendL += aRvbL
    gasendR += aRvbR
endin

instr    5 ; output
              outs       gasendL, gasendR    ; SEND UN-CHORUSSED AUDIO TO THE OUTPUTS
...
; ktrig        init       1
 if ktrig==1 then
  ...
  #
  if i(gkInstr)==$N then
   cabbageSetValue    "AAtt", $AAtt  
   cabbageSetValue    "ADec", $ADec  
   cabbageSetValue    "ASus", $ASus
   cabbageSetValue    "ARel", $ARel  
   cabbageSetValue    "FAmt", $FAmt  
   cabbageSetValue    "FDec", $FDec  
   cabbageSetValue    "FSus", $FSus  
   cabbageSetValue    "FRel", $FRel
   cabbageSetValue    "F_OS", $F_OS
   cabbageSetValue    "ChoMix", $ChoMix
   cabbageSetValue    "ChoDep", $ChoDep
   cabbageSetValue    "ChoRte", $ChoRte
  endif
  #
