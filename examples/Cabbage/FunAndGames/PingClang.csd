...
<Cabbage>
form caption("Ping Clang") size(610,500), guiMode("queue"), pluginId("PnCl")
...
image bounds( 0, 0, 0,0), shape("sharp"), colour(255,255, 50), channel("block1")    ; blocks: yellow
image bounds( 0, 0, 0,0), shape("sharp"), colour( 75,255, 75), channel("block2")    ; green
image bounds( 0, 0, 0,0), shape("sharp"), colour( 65, 65,255), channel("block3")    ; blue
image bounds( 0, 0, 0,0), shape("sharp"), colour(255, 15, 15), channel("block4")    ; red
image bounds( 0, 0, 0,0), shape("sharp"), colour(255,100, 50), channel("block5")    ; orange
image bounds( 0, 0, 0,0), shape("sharp"), colour(155, 50,255), channel("block6")    ; purple
image bounds( 0, 0, 0,0), shape("sharp"), colour(  0,230,255), channel("block7")    ; turquoise
image bounds( 0, 0, 0,0), shape("sharp"), colour(255,  0,255), channel("block8")    ; pink
image bounds( 0, 0, 0,0), shape("sharp"), colour(155,155,155), channel("block9")    ; grey
                                                             
image bounds(0,0,0,0), shape("ellipse"), colour(255,230,220), channel("ball")    ; ball
...
label      bounds(425,483,15,14), text("5"), align("centre"), colour(black), fontColour("white"), channel("NBlocksID")

label    bounds(  0, 0,  0, 0), text("Use left click and drag to move blocks."), align("centre"), fontColour("white"), channel("instructions1")        ; Instructions
label    bounds(  0, 0,  0, 0), text("Right click and drag to resize blocks."), align("centre"), fontColour("white"), channel("instructions2")        ; Instructions
...
instr    1    ; Track mouse position and clicks and move objects as appropriate
 ...
 kMOUSE_DOWN_MIDDLE cabbageGetValue    "MOUSE_DOWN_MIDDLE"
 ...
 kNBlocksDn        cabbageGetValue     "NBlocksDn"                     ; read in widgets
 kNBlocksUp        cabbageGetValue     "NBlocksUp"                     ;
 ...
 gkdamping         cabbageGetValue     "damping"
 gkfriction        cabbageGetValue     "friction"
 gkspeed           cabbageGetValue     "speed"
 gksize            cabbageGetValue     "size"
 gkangle           cabbageGetValue     "angle"
 gkresonance       cabbageGetValue     "resonance"
 gkfundemental     cabbageGetValue     "fundemental"
 gkNewBall         cabbageGetValue     "NewBall"
 ...
 if ((kMOUSE_DOWN_LEFT_trig==1||kMOUSE_DOWN_RIGHT_trig==1) && kMOUSE_X>=(gkLocX$N-1) && kMOUSE_X<=(gkLocX$N+gkSizeX$N+1) && kMOUSE_Y>=(gkLocY$N-1) && kMOUSE_Y<=(gkLocY$N+gkSizeY$N+1)) || kMoveFlag$N==1 then    ; If we are in click and drag mode
  ...
  if changed(gkDeltaX,gkDeltaY)==1 then                                ; if a change in mouse position during click
   ...
   kSizeY$N        =                   (kcount>gkNBlocks?0:gkSizeY$N)
                   cabbageSet          1, "block$N", "bounds", gkLocX$N, gkLocY$N, kSizeX$N, kSizeY$N
  endif
...
instr    2
 ...
 if ktrig==1 then
  ...
  SKIP:                                                                                                      ; Escape to here if finished checking for block collisions
                   cabbageSet          1, Sid, "bounds", kx, ky, isize, isize
...
instr    1001    ; Print and then hide instructions
 ...
 if metro(ksmps)==1||krel==1 then                                          ; peg updates or interrupt if mouse has been clicked
  ...
  Smess2           sprintfk            "bounds(%d,%d,400,17), fontColour(255,255,255,%d)", (giPanelWidth*0.5) - 200, (giPanelHeight*0.5) - 25, kalpha*(1-krel)
                   ...
                   cabbageSet          1, "instructions2", Smess2
 endif
