...
<Cabbage>
form caption("Air Raid Siren") size(830,184), colour(120,120,150), pluginId("ARSi"), guiMode("queue")
image                 bounds( 4, 4, 822,177), corners(15), outlineColour(50,50,50,150), outlineThickness(3) shape("square"), colour(190,190,210)
image                 bounds( 8, 8, 814,169), corners(15), outlineColour(50,50,50,150), outlineThickness(3) shape("square"), colour(190,190,210)
...
;}
image      bounds( 700,47,100,90), colour(70,65,70,155), outlineThickness(4), outlineColour("black") 

image    bounds( 16, 16, 68, 28), shape("rounded"), colour(255,100,100,60), channel("OnGlow"), visible(0)
button   bounds( 20, 20, 60, 20), fontColour:0(100,100,100), fontColour:1(255,205,205), colour:0(10,0,0), colour:1(200,50,50), text("On","On"), channel("On"), latched(0), value(0) ;, identChannel("OnID")

image    bounds( 19, 42, 21, 21), shape("ellipse"), colour(255,255,100,40), channel("LatchGlow"), visible(1)
...
gentable bounds(490, 25,90,50), channel("table1"), tableNumber(101), fill(0), outlineThickness(2), tableColour:0("lightBlue"), ampRange(-1.1,1.1,101)
...
;image      bounds( -100,-100, 10, 40), colour(55, 55, 55), widgetArray("Fins2", 10), shape("sharp"), outlineThickness(2), outlineColour("black")
 ...
 iCount            =                   0
 while iCount < 10 do
 SWidget           sprintf             "bounds( -100,-100, 10, 40), colour(55, 55, 55), channel(\"Fins1_ident%d\", 10), outlineThickness(2), outlineColour(\"black\")", iCount
                   cabbageCreate       "image", SWidget
 SWidget           sprintf             "bounds( -100,-100, 10, 40), colour(55, 55, 55), channel(\"Fins2_ident%d\", 10), outlineThickness(2), outlineColour(\"black\")", iCount
                   cabbageCreate       "image", SWidget
 iCount            +=                  1
 od


                   cabbageCreate       "image", "channel(\"Mask1\"), bounds(680,47,20,90), colour(190,190,210)"
                   cabbageCreate       "image", "channel(\"Mask2\"), bounds(800,47,20,90), colour(190,190,210)"

kOn        cabbageGetValue         "On"                                    ; the basic on/off values, only ever 1 (on) or zero (off)
kLatch     cabbageGetValue         "Latch"
if changed:k(kLatch)==1 then
 Smsg      sprintfk       "latched(%d)", kLatch
           ;chnset         Smsg, "OnID"
           cabbageSet     1, "On", Smsg           
 Smsg      sprintfk       "visible(%d)", kLatch
           ;chnset         Smsg, "LatchGlow"
           cabbageSet     1, "LatchGlow", Smsg           
endif

;if changed:k(kOn)==1 then
; Smsg      sprintfk       "visible(%d)", kOn
;           chnset         Smsg, "OnGlow"
           cabbageSet     changed:k(kOn), "OnGlow", "visible", kOn
;endif

kLevel     cabbageGetValue         "Level"
kSpeed     cabbageGetValue         "Speed"
kRiseDamp  cabbageGetValue         "RiseDamp"
kFallDamp  cabbageGetValue         "FallDamp"
kClip      cabbageGetValue         "Clip"
kSkew      cabbageGetValue         "Skew"
kHornFrq   cabbageGetValue         "HornFrq"
kPortTime  linseg         0, 0.001, 0.05
kHornFrq   portk          kHornFrq, kPortTime
kHornRes   cabbageGetValue         "HornRes"
kLoCut     cabbageGetValue         "LoCut"
kHiCut     cabbageGetValue         "HiCut"
kRatio     =              giRatios[cabbageGetValue:k("Ratio")-1]
kPan       cabbageGetValue         "Pan"                                  ; pan position, 0 to 1
kDist      cabbageGetValue         "Dist"                                 ; distance from listener. As this increases the reverb gets louder and the dry signal get quieter to simoulate distance     
...
kMinSweep  cabbageGetValue         "MinSweep"
...
aPtr         phasor        kCPS
             ...
             cabbageSet    changed:k(kClip,kSkew,kMinSweep), "table1", "tableNumber", 101
...
iRvbSend = 2
           ...
           cabbageSetValue   "VU_SPEED", kCPS/2000, metro:k(24)
...
;             chnset         kres, "VU_LEVEL"
           cabbageSetValue   "VU_LEVEL", kres, metro:k(24)


if changed:k(cabbageGetValue:k("Ratio"))==1 then

 kCnt        =              1
 while kCnt<=10 do
 SID         sprintfk       "Fins1_ident%d", kCnt
;             chnset         "pos(600,-53)", SID
             cabbageSet     1, SID, "bounds", 600, -53, 0, 0
...
if metro(30)==1 then
 kCnt        =              1
 ;kNFins     =              5 
 kNFins      =              giRatioN[cabbageGetValue:k("Ratio")-1]
 kWid        =              100/kNFins
 kWid        *=             (100+kWid)/100
 while kCnt<=kNFins do
 kPtr1       wrap           kPtr + ((kCnt-1)/kNFins), 0, 1
 Smsg        sprintfk       "bounds(%d,%d,%d,%d), colour(%d,%d,%d)", 700+((kPtr1*100)-kWid) * (100+kWid)/100, 47+4, kWid - 2, 40, kPtr1*150, kPtr1*150, kPtr1*150
 SID         sprintfk       "Fins1_ident%d", kCnt
             ;chnset         Smsg, SID
 ...
 kCnt        =              1
 ;kNFins     =              6
 kNFins      =              giRatioD[cabbageGetValue:k("Ratio")-1]
 kWid        =              100/kNFins
 kWid        *=             (100+kWid)/100
 while kCnt<=kNFins do
 kPtr1       wrap           kPtr + ((kCnt-1)/kNFins), 0, 1
 Smsg        sprintfk       "bounds(%d,%d,%d,%d), colour(%d,%d,%d)", 700+((kPtr1*100)-kWid)  * (100+kWid)/100, 47+46, kWid - 2, 40, kPtr1*150, kPtr1*150, kPtr1*150
 SID         sprintfk       "Fins2_ident%d", kCnt
             ;chnset         Smsg, SID
             cabbageSet     1, SID, Smsg             
 kCnt        +=             1
