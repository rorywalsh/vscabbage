...
<Cabbage>
form caption("Pebbles in a Pond"), size(900, 520), pluginId("Pond"),colour(0,10,30), guiMode("queue")
label bounds( 0, 0, 0, 0), text("Drop a pebble into the pond..."), align("centre"), fontColour("white"), channel("instructions")

label bounds(  5,508,100, 12), text("Iain McCurdy 2014"), align("left"), fontColour("Silver")
...
instr    1    ; create circle widgets, sense mouse activity and trigger note events for instr 2
 iNCircles         =                   70
 iCount            =                   1
 while iCount <= iNCircles do
 SWidget           sprintf             "bounds(0,0,0,0), shape(\"ellipse\") channel(\"circle_ident%d\")", iCount
 ...
 kMOUSE_DOWN_LEFT cabbageGetValue    "MOUSE_DOWN_LEFT"   ; Read in mouse left click status
 ...
 if gkRefreshTrig==1 then                                                            ; If a trigger has been generated...  
  SCircle         sprintf    "circle_ident%d",giCircleNdx
   Smessage       sprintfk   "bounds(%d,%d,%d,%d),colour(%d,%d,%d,%d)", kHorPos, kVertPos, kSize, kSize, iRed, iGreen, iBlue, kAlpha    ; Create a string with updated attributes that will be sent to the circle widget.
                  cabbageSet 1, SCircle, "bounds", kHorPos, kVertPos, kSize, kSize
                  cabbageSet 1, SCircle, "colour", iRed, iGreen, iBlue, kAlpha
...
instr    1000    ; Print initial text instructions, expanding and disappearing.
 ...
 iSizeFinal       =          80                                    ; Final size (in pixels)
 kSize            linseg     iSizeInit,p3-0.001,iSizeFinal,0.001,0 ; Text Size increasing as time progresses
  
 kAlpha           transeg    0,0.01,4,1,p3-0.01,-8,0,1,0,0         ; Colour alpha decreases as event progresses, i.e. transparency increases
 
 ktrig            metro      kr/4                                 ; Create a metronome which will define the rate of graphical update of the text
                  cabbageSet ktrig, "instructions", "bounds", -200, 245, giPanelSizeX+400, kSize
                  cabbageSet ktrig, "instructions", "alpha", kAlpha
...
<CsScore>
i 1 0 z       ; Instrument to sense mouse activity
i 99 0 z      ; Reverb instrument
i 1000 0 8    ; Instructions
</CsScore>
