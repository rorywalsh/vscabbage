...
<Cabbage>
form  size(1000,540), caption("Electricity2"), guiMode("queue"), colour(0,0,0), pluginId("Elec")
label bounds(0,530,100,10), text("Iain McCurdy |2015|")
label bounds(  0,230,1000,70), text("Click and Drag..."), fontColour(255,255,255), channel("instructions")
</Cabbage>
...
instr 1
 ...
 gkMOUSE_X    cabbageGetValue   "MOUSE_X"                  ; read in mouse x position in pixels
 gkMOUSE_Y    cabbageGetValue   "MOUSE_Y"                  ; read in mouse y position in pixels
 ...
 kMOUSE_DOWN_LEFT cabbageGetValue  "MOUSE_DOWN_LEFT"       ; mouse left click status (0 or 1)
...
instr 11 ; show spark and move and set colour
 Sident         sprintf     "spark%d",p4                           ; define ident channel string
 ...
 Smsg           sprintf     "bounds(%d,%d,%d,%d), rotate(%f,0,0), colour(255,255,%d,%d), visible(1)",i(gkMOUSE_X),i(gkMOUSE_Y),ilen,rnd(2)+1,irad,255-icol,ialpha
                cabbageSet  Sident, Smsg
                event_i     "i", 12, 0.04, 0, p4                   ; call instrument to 'hide' spark after a short delay
endin

instr 12 ; hide spark
                Sident      sprintf "spark%d",p4                   ; define indent channel string
                cabbageSet  Sident, "visible", 0
endin
...
instr 1000  ; create spark widgets and flash instructions
 iNSparks          =                   10
 iCount            =                   1
 while iCount <= iNSparks do
 SWidget           sprintf             "bounds(0,0,0,0) channel(\"spark%d\")", iCount
 ...
 if changed(kvis)==1 then                                             ; if flash changing from on to off or vice versa
  Smsg          sprintfk    "visible(%d), fontColour(255, 255, 255, %d)", kvis, kcol ; create string
                cabbageSet  1, "instructions", "visible", kvis
                cabbageSet  1, "instructions", "fontColour", 255, 255, 255, kcol
 endif
 ...
 kMOUSE_DOWN_LEFT cabbageGetValue  "MOUSE_DOWN_LEFT"                 ; sense mouse left click
 if kMOUSE_DOWN_LEFT==1 then                                ; if left clicked...
                cabbageSet  1, "instructions", "visible", 0
