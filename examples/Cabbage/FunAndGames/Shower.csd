...
form caption("Shower") size(510, 240), pluginId("Shwr") colour(20,25,30), guiMode("queue")

encoder    bounds( 10 , 50, 120, 120), text("TAP"),  channel("DripsDens"),  increment(1), repeatInterval(1000) range(0,3000,3), popupText(0) $RSliderStyle2
...
label bounds(  2,227,100, 12), text("Iain McCurdy 2021"), align("left"), fontColour("Grey")
</Cabbage>
...
instr  1
 ...
 iNDrips           =                   500
 iCount            =                   1
 while iCount <= iNDrips do
 SWidget           sprintf             "bounds(0,0,0,0), shape(\"ellipse\") channel(\"Drip%d\")", iCount
                   cabbageCreate       "image", SWidget
 iCount            +=                  1
 od

; READ IN WIDGETS
kDrips             cabbageGetValue     "Drips"
gkDripsDens        cabbageGetValue     "DripsDens"
gkDripsDens        limit               gkDripsDens,0,600
kDripsDur          cabbageGetValue     "DripsDur"
kDripsDur          *=                  0.01
kDripsDur          =                   kDripsDur * 2 ^ (exprand:k(0.3))
kDripsPitch        cabbageGetValue     "DripsPitch"
kDripsDiverg       cabbageGetValue     "DripsDiverg"
gkDripNum          init                1
kTrig              dust                1, limit:k(gkDripsDens,0,200)
...
; Draw drips
instr 2
kHeight            line                0, p3, 265
iRange             trirand             10 * limit:i(i(gkDripsDens)^0.25, 0, 50)
iX                 =                   200
kX                 transeg             iX, p3, -4, iX + iRange
iSize              random              2, 6
SID                sprintfk            "Drip%d", p4
                   cabbageSet          metro:k(48), SID, "bounds", kX, kHeight, iSize, iSize
gkDripNum          init                (p4 + 1) % 500
endin
...
kpos               limit               (kpos + kPosMod)* kChange, 0.2, 1
/* BOW PRESSURE */    
kbowpres           =                   1.2
...
gkgain             =                   0.9
gkconst            =                   0.999
abowedbar          wgbowedbar          k(0), kcps, kpos, kbowpres, gkgain, i(gkconst), 0, 0, 1
    
/* OUTPUT LEVEL CONTROL */
abowedbar          *=                  0.05 * kChange
