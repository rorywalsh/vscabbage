...
<Cabbage>
form caption("Splat!"), size(1100,530), colour(230,230,230), pluginId("Splt"), guiMode("queue")
nslider  bounds(  5,495,60,25), channel("Population"),    range(1,200, 30,1,1),       textColour("black"), fontColour("black"), text("Population"), colour("white")
...
label    bounds(350,495,70,9), text("Distribution"), fontColour("black")
combobox bounds(350,505,70,15), channel("Distr"), items("jspline","randomi","gaussi"), value(1), textColour("black"), fontColour("black"), colour:0("white"), align("centre")
label    bounds(  0, 0,  0, 0), text("Click swarm to splat. Click again to unsplat."), align("centre"), fontColour("white"), channel("instructions1")
label    bounds(995,519,100, 10), text("Iain McCurdy 2014"), align("right"), fontColour(0,0,0,150)
</Cabbage>
...
opcode  Fly, aa, kkkkkkkkip
 kSplat, kExcursion, kLiveliness, koct, kFlySize, kOffX, kOffY, kDistr, iMaxFlies, iCount    xin
 ...
 if kSplat==0 then                                                               ; if flies are living...
  ...
  elseif kDistr==2 then
   kDisp           randomi             -kExcursion, kExcursion, kLiveliness  ; variables for fly movement within the swarm
  else
   kDisp           gaussi              kExcursion,kExcursion*0.017,kLiveliness
  endif
  kRotation        jspline             2 * $M_PI, kLiveliness * 0.2, kLiveliness * 0.4    ; angle liveliness should be less
  ...
  if kRefreshTrig==1 then                                                                 ; if a refresh trigger has occured...
      Smess        sprintfk            "bounds(%d,%d,%d,%d), colour(0,0,0)",kOffX+kX,kOffY+kY,kDist*kFlySizeL,kDist*kFlySizeL    ; generate message for fly widget
      Sident       sprintfk            "fly_ident%d", iCount                              ; generate channel string appropriate to this fly
                   cabbageSet          1, Sident, Smess                                   ; update the fly
  endif
 endif
 ...
 kenv              linseg              0,0.02,0,0.02,1
 asig              vco2                0.01*kamp*kenv,cpsoct(koct)*semitone((kX+kY)*0.02),6 ; the buzzing sound: a pulse wave
 ...
 aMixL             =                   0                                ; clear mix audio variables
 aMixR             =                   0

 if iCount<iMaxFlies then
  aMixL,aMixR      Fly                 kSplat,kExcursion,kLiveliness,koct,kFlySize,kOffX,kOffY,kDistr,iMaxFlies,iCount+1
 endif
...
instr    1
 ...
 kMOUSE_X          cabbageGetValue     "MOUSE_X"                  ; track mouse activity
 ...
 kLiveliness       cabbageGetValue     "Liveliness"               ; speed with which flies move within the swarm 
 kExcursion        cabbageGetValue     "Excursion"                ; range (in pixels) that individual flies might go from the the centre of the swarm
 kFlySize          cabbageGetValue     "FlySize"                  ; fly size (affects pitch of buzzing only)
 ...
 kSwarmSpeed       cabbageGetValue     "SwarmSpeed"
 kDistr            cabbageGetValue     "Distr"
 kDistr            init                3
 if kSplat==0 then                                   ; if flies are alive...
  gkPopulation     cabbageGetValue     "Population"  ; read changes to population. This widget is only read while flies are alive so that splatted flies don't appear or disappear while splatted.
 endif
 ...
 ChangedPopulation:
 aMixL,aMixR       Fly                 kSplat,kExcursion,kLiveliness,koct,kFlySize,kOffX,kOffY,kDistr,i(gkPopulation)
 rireturn
...
instr    10    ; hide all flies
 ...
 Sid               sprintf             "fly_ident%d",iCount
                   cabbageSet          Sid, "bounds",-100,-100,0,0
                   cabbageSet          Sid, "colour",0,0,0,0
 iCount            +=                  1
 od
endin
...
instr    1001    ; Print and then hide instructions
 iCount            =                   1
 while iCount <= 500 do
 SWidget           sprintf             "bounds(0,0,0,0), colour(\"white\"), shape(\"ellipse\"), channel(\"fly_ident%d\")", iCount
                   cabbageCreate       "image", SWidget
 iCount            +=                  1
 od

  kMOUSE_DOWN_LEFT  cabbageGetValue     "MOUSE_DOWN_LEFT"                  ; sense left click
 ...
 if metro(ksmps)==1||krel==1 then                      ; peg updates or interrupt if mouse has been clicked
  Smess1           sprintfk            "bounds(%d,%d,400,17), fontColour(0,0,0,%d)", (giPanelWidth*0.5) - 200, (giPanelHeight*0.5)-10, kalpha*(1-krel)
                   cabbageSet          1, "instructions1", Smess1
 endif
