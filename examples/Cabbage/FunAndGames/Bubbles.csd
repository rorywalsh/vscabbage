...
<Cabbage>
form caption("Bubbles"), size(1000, 540), pluginId("Bubl"), colour(170,200,255), guiMode("queue")
label bounds(  0,250,1000, 30), text("Burst the Bubbles..."), align("centre"), fontColour(0,0,0,0), identChannel("instructions")
label    bounds(  5,  2, 45, 15), text("Burst:"), align("left"), fontColour(0,0,0,100)
label    bounds( 50,  2, 40, 15), text("0"), align("left"), fontColour(0,0,0,150), channel("burstID")
label    bounds( 90,  2, 55, 15), text("Missed:"), align("left"), fontColour(0,0,0,100)
label    bounds(145,  2, 40, 15), text("0"), align("left"), fontColour(0,0,0,150), channel("missedID")
...
instr 1
 ...
 gkMOUSE_X         cabbageGetValue   "MOUSE_X"           ; Read in mouse X position (in pixels across the panel)
 gkMOUSE_Y         cabbageGetValue   "MOUSE_Y"           ; Read in mouse Y position (in pixels across the panel)
...
instr $INSTR
 ...
 if gkRefreshTrig==1 then                                               ; If a trigger has been generated...
  ...
  Smessage      sprintfk    "bounds(%d,%d,%d,%d), colour(255,255,255,%d)", kx$N+(ksize_x*0.6), ky+(ksize_y*0.22), ksize_x*0.2, ksize_y*0.2,40 ; Create a string with updated attributes that will be sent to the widget.
                cabbageSet  1, Sspecular, Smessage
 ...
 if ktrig==1 then
  ...
  if kdist<=(isize_x*0.5) then                                                                                                          ; if distance of mouse click is less than or equal to radius of the bubble...
   ...
   ktrig        metro       32
                cabbageSet  ktrig, Sbubble, "outlineThickness", 1
                cabbageSet  ktrig, Sbubble, "colour", 0,0,0,0
                cabbageSet  ktrig, Sbubble, "outlineColour", 240, 240, 240                
                cabbageSet  ktrig, Sspecular, "bounds", 0, 0, 0, 0
   turnoff
  endif
 endif 
endin
...
instr 1000 ; create widgets and reverb
; create bubbles widgets
 iCount            =                   1
 while iCount <= 10 do
 SWidget           sprintf             "bounds(  0,  0,  0, 0), colour(0,0,0), shape(\"ellipse\"), channel(\"bubble_ident%d\")", iCount
                   cabbageCreate       "image", SWidget
 SWidget           sprintf             "bounds(  0,  0,  0, 0), colour(0,0,0), shape(\"ellipse\"), channel(\"specular_ident%d\")", iCount
                   cabbageCreate       "image", SWidget
 iCount            +=                  1
 od
...
instr 2000 ; Print initial text instructions then fading away.
 ...
 if ktrig==1  then                                                                            ; If a trigger has been generated...
   Smess1     sprintfk      "fontColour(0,0,50,%d)",kAlpha                                    ; Create a string with updated attributes that will be sent to the instructions widget.
              cabbageSet    1, "instructions", Smess1
 endif
...
<CsScore>
i 1000 0 z ; Create widgets and Reverb
