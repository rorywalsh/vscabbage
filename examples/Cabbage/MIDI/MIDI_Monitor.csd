...
; The fourth box provides a graphical impression of a selected continuous controller
; The user can select the channel number and the controller number of the controller to be displayed

<Cabbage>
form caption("MIDI Monitor"), size(610, 590), colour( 50, 50, 50), pluginId("MiMo"), guiMode("queue")
...
; DISPLAY A SELECTED CONTINUOUS CONTROLLER
label     bounds(  5,352, 22, 12), text("127"), align("right")
label     bounds(  5,407, 22, 12), text("64"), align("right")
label     bounds(  5,462, 22, 12), text("0"), align("right")
gentable  bounds( 30,355,570,115), channel("graph"), tableNumber(99), tableColour(255,150,150), fill(0), ampRange(0,127,99),  tableGridColour(100,100,100,50) ;, tableBackgroundColour(50,50,50)
image     bounds( 30,355,  1,115), channel("wiper"), colour(255,150,150)

nslider   bounds( 30,469, 40, 23), channel("val"), range(0,127,0,1,1), fontColour("Silver"), colour(0,0,0,100)
label     bounds( 60,475, 80, 12), text("Channel:"), fontColour("white"), align("right")
nslider   bounds(145,469, 35, 23), channel("graphChan"), range(1,16,1,1,1), fontColour("Silver"), colour(0,0,0,100)
label     bounds(185,475,120, 12), text("Controller Number:"), fontColour("white"), align("right")
nslider   bounds(310,469, 35, 23), channel("graphCCNum"), range(0,127,1,1,1), fontColour("Silver"), colour(0,0,0,100)
label     bounds(395,475, 70, 12), text("Update Rate:"), fontColour("white"), align("right")
nslider   bounds(470,469, 35, 23), channel("graphUpdate"), range(0.001,1,0.1,1,0.001), fontColour("Silver"), colour(0,0,0,100)

keyboard bounds( 10,495,590, 80)

label    bounds( 10,577,110, 12), text("Iain McCurdy |2014|"), align("left"), fontColour("Silver")
...
giBufTab          ftgen            99, 0, 1024, 2, 0 ; graph table
...
instr 2
 ...
 kgraphChan       cabbageGetValue  "graphChan"
 kgraphChan       init             1
 kgraphCCNum      cabbageGetValue  "graphCCNum"
 if changed:k(kgraphChan,kgraphCCNum)==1 then
                  reinit           RESTART_CTRL7
 endif
 RESTART_CTRL7:
 kVal             ctrl7            i(kgraphChan), i(kgraphCCNum), 0, 127
 rireturn
 kUpdateTrig      metro            32
 kPhasor          phasor           cabbageGetValue:k("graphUpdate")
                  tablew           kVal,kPhasor,99,1                                         ; write to table
                  cabbageSet       kUpdateTrig,"graph","tableNumber",99                      ; write to gentable
                  cabbageSet       kUpdateTrig,"wiper","bounds",30 + (kPhasor*570),355,1,115 ; move wiper
                  cabbageSetValue  "val",kVal,changed:k(kVal)                                ; write to number box
endin

</CsInstruments>

<CsScore>
i 2 0 z        ; instrument to update function tables and table displays
</CsScore>                            
