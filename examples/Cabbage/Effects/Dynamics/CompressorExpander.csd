...
; Smoothing     -   A lowpass filter is applied to the output gain function. Increasing 'Smoothing' will apply a lag to changes in dynamics.
...
<Cabbage>
form caption("Compressor/Expander") size(890, 100), pluginId("CoEx"), guiMode("queue")
...
image    bounds(297,  7, 86, 86), outlineThickness(2), colour("DarkGrey"), outlineColour("DarkGrey"), corners(6)
gentable bounds(300, 10, 80, 80), tableNumber(1), tableColour("silver"), fill(0), channel("table"), ampRange(0,1,-1), zoom(-1), tableGridColour(100,100,100,100), tableBackgroundColour("white"), outlineThickness(2)
image    bounds(0, 0, 6, 6), colour("Red"), shape("ellipse"), outlineThickness(0), channel("Dot")
...
label    bounds(  5, 87,120, 10), text("Iain McCurdy |2015|"), align("left"), fontColour("darkGrey")
...
instr 1
 ...
 kPreGain          cabbageGetValue     "PreGain"
 ksense            cabbageGetValue     "sense" 
 ...
 if metro(ksmps*0.5)==1 then
  ...
  ;Smsg             sprintfk            "pos(%d,%d)", 297 + int(kx * 80), 7 + (80 - int(ky * 80)) ; create message string
                   cabbageSet          1, "Dot", "bounds", 297 + int(kx * 80), 7 + (80 - int(ky * 80)), 6, 6                                                ; send message string to gentable widget
 endif
