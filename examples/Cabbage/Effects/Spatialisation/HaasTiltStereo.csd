...
; A delay time is calculated for each of two speakers based on how far away each is from a movable source sound widget.
...
; The 'DELAY TIMES (ms)' box contains two number boxes which display the delay time applied at each speaker.
;  These are for display only.

; The 'AMPLITUDE (dB)' box contains two number boxes which display the amplitude attenuation applied at each speaker.
;  These are for display only.

; The "SPEAKER LOCATIONS' box contains four number boxes which define the location of each of the two speakers.
...
;                   Stereo in which the left input channel is sent to speakers 1 and the right input channel
;                   is sent to speakers 2, or a test beeper.
...
<Cabbage>
form caption("Haas Tilt Stereo") size(760,470), pluginId("Spt8"), guiMode("queue"), colour(40,40,40)
...
; LABEL
label bounds( 0,140,400,120), text("H A A S"), fontColour("white"), alpha(0.05), colour(0,0,0,0)
...
nslider  bounds(  5, 25, 70, 35), channel("X1"), range(-1,1,-0.5,1,.01), text("Spk.1 X")
nslider  bounds( 85, 25, 70, 35), channel("Y1"), range(-1,1, 0.5,1,.01), text("Spk.1 Y")
nslider  bounds(185, 25, 70, 35), channel("X2"), range(-1,1, 0.5,1,.01), text("Spk.2 X")
nslider  bounds(265, 25, 70, 35), channel("Y2"), range(-1,1, 0.5,1,.01), text("Spk.2 Y")
}

image    bounds(410,165,390,210), colour(0,0,0,0)
{
label    bounds(  0,  0, 80, 14), text("INPUT")
combobox bounds(  0, 15, 80, 20), channel("MonoStereo"), items("Mono","Stereo","Beeper"), value(1) 
hslider  bounds(  0, 55,340, 20), channel("Smoothing"), text("Smoothing"), range(0.01,0.3,0.05,0.5,0.0001), valueTextBox(1)
hslider  bounds(  0, 85,340, 20), channel("DelRange"), text("Delay Range"), range(1, 60,10,1,0.1), valueTextBox(1)
hslider  bounds(  0,115,340, 20), channel("AmpDropOff"), text("Amp.Drop-Off"), range(-3, 24,12,1,0.1), valueTextBox(1)
checkbox bounds(  0,155,100, 15), channel("FilterOnOff"), value(1), text("Filter On/Off")
hslider  bounds(  0,185,340, 20), channel("FilterDropOff"), text("Filter Drop-Off"), range(0, 8, 4, 1, 0.01), valueTextBox(1)
}

; meters
image   bounds(500,170, 35, 42), colour(0,0,0,0)
{
...
; file player
image bounds( 10,410,800,200) colour(0,0,0,0)
{
...
soundfiler bounds( 80,  0,660, 50), channel("beg","len"), channel("filer1"),  colour(0, 255, 255, 255), fontColour(160, 160, 160, 255)
...
instr 1
 ...
 if kMonoStereo==1 then     ; mono
  aL    inch   1
 elseif kMonoStereo==2 then ; stereo
  aL,aR ins
 else                       ; beeper
  kamp  lfo    0.2,8,3
  aL    poscil a(kamp), 880
  aR    =      aL
 endif
...
; clear input audio variables if sound file input is stopped
 aL     =      0
 aR     =      0
