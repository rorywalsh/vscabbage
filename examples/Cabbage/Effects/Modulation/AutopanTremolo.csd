...
<Cabbage>
form caption("Autopan / Tremolo") size(565, 115), pluginId("aptr"), guiMode("queue")
#define SLIDER_STYLE valueTextBox(1), textBox(1), trackerColour(205,120,120), fontColour("lightGrey"), textColour("lightGrey")
image                   bounds( 0,  0, 565, 115), colour(55,20,20), shape("rounded"), outlineColour("white"), outlineThickness(4)

rslider  bounds(  5,  6, 90, 90), text("Freq.[Hz]"),  channel("rate"),  range(0.1, 50, 0.5, 0.5), $SLIDER_STYLE
rslider  bounds( 80,  6, 90, 90), text("Tempo[BPM]"), channel("tempo"), range(6, 3000, 30, 0.5, 1), $SLIDER_STYLE
rslider  bounds(175,  6, 90, 90), text("Depth"),      channel("depth"), range(0, 1.00, 1, 0.5), $SLIDER_STYLE

image    bounds(345,  5,140,140), colour(0,0,0,0), alpha(0.2), channel("user")
{
line     bounds( 15,  5,107,  2), colour("lightGrey")
label    bounds( 48,  0, 43, 12), text("USER"), colour(55,20,20), fontColour("lightGrey")
rslider  bounds(  0, 11, 80, 80), text("Bias"),      channel("bias"), range(-1, 1.00, 0), $SLIDER_STYLE
rslider  bounds( 60, 11, 80, 80), text("Duty"),      channel("duty"), range(0.01, 2000.00, 1, 0.25, 0.001), $SLIDER_STYLE
}

rslider  bounds(470,  6, 90, 90), text("Level"),      channel("level"), range(0, 1.00, 1), $SLIDER_STYLE

checkbox bounds(160, 40, 25, 25), colour("yellow"), channel("indicator"),  value(1), shape("ellipse"), active(0), fontColour:0("lightGrey"), fontColour:1("lightGrey")
combobox bounds(260, 13,  90,20), channel("mode"), value(1), text("Autopan", "Tremolo"), fontColour("lightGrey"), textColour("lightGrey")
combobox bounds(260, 38,  90,20), channel("wave"), value(1), text("Sine", "Triangle", "Square", "Randomi", "Randomh", "Rspline", "User"), fontColour("lightGrey"), textColour("lightGrey")
checkbox bounds(260, 63, 90, 15), text("TEST TONE"), colour("lime"), channel("test"),  value(0), fontColour("lightGrey"), fontColour:0("lightGrey"), fontColour:1("lightGrey")
label   bounds(  5,100,120, 12), text("Iain McCurdy |2012|"), align("left"), fontColour("silver")
</Cabbage>
...
opcode PanTrem, aa, aakkkKkK
    ...
    if kmode=0 then    ;PAN                                          ; IF PANNING MODE IS CHOSEN FROM BUTTON BANK...
        ...
        kindicator    =                  (klfo>0.5?1:0)
                      cabbageSet         metro:k(32), "indicator", "colour:1", 255*klfo, 255*klfo, 0
    elseif kmode==1 then                                             ; IF TREMOLO MODE IS CHOSEN FROM BUTTON BANK...
        kindicator    =                  (klfo>0.5?1:0)
                      cabbageSet         metro:k(32), "indicator", "colour:1", 255*klfo, 255*klfo, 0
...
kwave       cabbageGetValue    "wave"
; printk2 kwave
            cabbageSet         trigger:k(kwave,6.5,0), "user", "alpha", 1
            cabbageSet         trigger:k(kwave,6.5,1), "user", "alpha", 0.2
kbias       cabbageGetValue    "bias"
kduty       cabbageGetValue    "duty"
...
if ktrig==1 then
            cabbageSetValue    "tempo", krate*60
elseif ktrig2==1 then
            cabbageSetValue    "rate", ktempo/60
endif
