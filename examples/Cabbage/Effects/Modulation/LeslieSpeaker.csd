...
<Cabbage>
form caption("Leslie Speaker") size(600,190), pluginId("Lsli"), colour("Black"), guiMode("queue")
#define BUTTON_STYLE fontColour:0(140,125,125), fontColour:1(10,0,0), colour:0(45,20,20), colour:1(250,170,170)
image     bounds(  0,  0,260,190), outlineThickness(4), outlineColour("white"), file("DarkWood.jpg");, colour(75,50,50)
image     bounds(  0,  0,260,190), outlineThickness(4), outlineColour("silver"), colour(0,0,0,0)

button    bounds( 40,145, 60, 25), text("Slow","Slow"), channel("Slow"), latched(1), radioGroup(1), value(1), $BUTTON_STYLE
button    bounds(100,145, 60, 25), text("Stop","Stop"), channel("Stop"), latched(1), radioGroup(1), $BUTTON_STYLE
button    bounds(160,145, 60, 25), text("Fast","Fast"), channel("Fast"), latched(1), radioGroup(1), $BUTTON_STYLE

image     bounds( 115, 10,30,30), channel("Tweeter"), shape("ellipse"), colour(170,150,150)
image     bounds(  90, 55,80,80), channel("Woofer"),  shape("ellipse"), colour(170,150,150)

button bounds( 10,10,50, 18), channel("testSound"), text("TEST"), latched(1), $BUTTON_STYLE

image     bounds(260,0,340,190), outlineThickness(4), outlineColour("silver"), colour(75,50,50), file("DarkWood.jpg")
{
...
label   bounds(   5,175,120, 12), text("Iain McCurdy |2016|"), align("left"), fontColour("Silver")
</Cabbage>
...
instr    1    ; a basic MIDI triggered organ sound
 ...
 aMix         =               0
 aMix         +=              poscil:a(0.2,cpsmidinn(36),giAllOut)                         
 aMix         +=              poscil:a(0.2,cpsmidinn(48),giAllOut)                     
 aMix         +=              poscil:a(0.2,cpsmidinn(52),giAllOut)                     
 aMix         +=              poscil:a(0.2,cpsmidinn(55),giAllOut)                     
 aMix         +=              poscil:a(0.2,cpsmidinn(59),giAllOut)                     
 aMix         +=              poscil:a(0.2,cpsmidinn(60),giAllOut)
 aMix         +=              poscil:a(0.2,cpsmidinn(64),giAllOut)
 aMix         +=              poscil:a(0.2,cpsmidinn(67),giAllOut)
 aMix         +=              poscil:a(0.2,cpsmidinn(71),giAllOut)
              chnmix          aMix,"send"                                         ; mix into send 'channel'
endin

instr    99    ; a Leslie speaker effect
 kSlow       cabbageGetValue  "Slow"                                              ; read 'Slow' button widget
 kStop       cabbageGetValue  "Stop"                                              ; read 'Stop' button widget
 kFast       cabbageGetValue  "Fast"                                              ; read 'Faset' button widget
 ...
 kCF_High     cabbageGetValue "CF_High"                                           ; crossover cutoff frequencies
 kCF_Low      cabbageGetValue "CF_Low"
 
 kHF_Slow     =               cabbageGetValue:k("HF_Slow")/60                     ; tweeter chorale (slow) speed
 kHF_Fast     =               cabbageGetValue:k("HF_Fast")/60                     ; tweeter tremolo (fast) speed

 kLF_Slow     =               cabbageGetValue:k("LF_Slow")/60                     ; tweeter chorale (slow) speed
 kLF_Fast     =               cabbageGetValue:k("LF_Fast")/60                     ; woofer tremolo (fast) speed
 
 kHF_Damp     cabbageGetValue "HF_Damp"                                           ; tweeter speed change time
 kLF_Damp     cabbageGetValue "LF_Damp"                                           ; woofer speed change time
 
 kHF_Trem     cabbageGetValue "HF_Trem"                                           ; tremolo modulation depth
 kLF_Trem     cabbageGetValue "LF_Trem"
 
 kHF_Dop      cabbageGetValue "HF_Dop"                                            ; doppler modulation depth
 kLF_Dop      cabbageGetValue "LF_Dop"
 
 kHF_Pan      cabbageGetValue "HF_Pan"                                            ; panning modulation depth
 kLF_Pan      cabbageGetValue "HF_Pan"
 
 kHF_Amp      cabbageGetValue "HF_Amp"                                            ; amplitude modulation depth
 kLF_Amp      cabbageGetValue "LF_Amp"
 ...
 ktrig        metro           16
              cabbageSet      ktrig,"Tweeter","bounds",115+(15*(1-kPhsTweeter)), 10, kPhsTweeter*30, 30
              cabbageSet      ktrig,"Tweeter","colour",kPhsTweeter*120+50, kPhsTweeter*100+50, kPhsTweeter*100+50  
              cabbageSet      ktrig,"Woofer","bounds", 90+(40*(1-kPhsWoofer)),55,kPhsWoofer*80,80
              cabbageSet      ktrig,"Woofer","colour", kPhsWoofer*120+50, kPhsWoofer*100+50, kPhsWoofer*100+50         
