...
; I N P U T
; MONO/STEREO - mono selects left channel and sends it to both channels of the stereo effect
...
<Cabbage>
form caption("Shimmer Reverb") size(1290,145), colour(20,20,20) pluginId("ShRv"), guiRefresh(16) 

#define DIAL_STYLE  trackerInsideRadius(0.8),  trackerColour(200,200,150), colour(160,160,170), textColour(200,200,150)

image    bounds( 10, 10,190,125), colour(0,0,0,0), outlineThickness(1) 
{
label    bounds(  0,  5,190, 15), text("I N P U T"), fontColour(200,200,150)
listbox  bounds( 10,  45,70, 40), items("MONO","STEREO"), value(2), channel("Input"), align("centre")
rslider  bounds(100, 25, 70, 90), text("Input Gain"),  channel("InputGain"),  range(0,1,0.95), $DIAL_STYLE
}


image    bounds(210, 10,590,125), colour(0,0,0,0), outlineThickness(1) 
{
label    bounds(  0,  5,590, 15), text("R E V E R B"), fontColour(200,200,150)
rslider  bounds( 20, 25, 70, 90), text("Pre Delay"),  channel("PreDelay"),  range(0.01,1,0.01,0.5), $DIAL_STYLE
rslider  bounds(100, 25, 70, 90), text("Smear"),  channel("Smear"),  range(0.01,3,1,0.5), $DIAL_STYLE
rslider  bounds(180, 25, 70, 90), text("Reverb Time"),  channel("RvbTime"),  range(0.1,18,7,0.5), $DIAL_STYLE
rslider  bounds(260, 25, 70, 90), text("Damping"),  channel("Damping"),  range(200,18000,18000,0.5), $DIAL_STYLE
rslider  bounds(340, 25, 70, 90), text("Mod.Rate"),  channel("ModRate"),  range(0.1,6,1,0.5), $DIAL_STYLE
rslider  bounds(420, 25, 70, 90), text("Mod.Depth"),  channel("ModDep"),  range(0,10,1,0.5), $DIAL_STYLE
rslider  bounds(500, 25, 70, 90), text("Size"),  channel("Size"),  range(0.1,10,1,0.5), $DIAL_STYLE
}

image    bounds(810, 10,190,125), colour(0,0,0,0), outlineThickness(1) 
{
label    bounds(  0,  5,190, 15), text("P I T C H   S H I F T"), fontColour(200,200,150)
rslider  bounds( 20, 25, 70, 90), text("Interval"),  channel("Interval"),  range(-24,24,12,1,0.1), $DIAL_STYLE
rslider  bounds(100, 25, 70, 90), text("Delay Time"),  channel("DelTime"),  range(0.05,5,0.5,0.5), $DIAL_STYLE
}

image    bounds(1010, 10,270,125), colour(0,0,0,0), outlineThickness(1) 
{
label    bounds(  0,  5,270, 15), text("M I X"), fontColour(200,200,150)
rslider  bounds( 20, 25, 70, 90), text("Pitch Mix"),  channel("PitchMix"),  range(0,1,1), $DIAL_STYLE
rslider  bounds(100, 25, 70, 90), text("Dry/Wet"),  channel("DryWet"),  range(0,1,0.5), $DIAL_STYLE
rslider  bounds(180, 25, 70, 90), text("Level"),  channel("Level"),  range(0,4,1.5,0.5), $DIAL_STYLE
}

;image    bounds( 10, 10, 90,125), colour(0,0,0,0), outlineThickness(1) 
label   bounds(  10,112, 90, 11), text("Iain McCurdy"), align("centre"), fontColour("Silver")
label   bounds(  10,123, 90, 11), text("|2021|"), align("centre"), fontColour("Silver")
</Cabbage>
