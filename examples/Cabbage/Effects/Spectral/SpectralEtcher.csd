...
; Applies a range of spectral procedures to a chosen sound file or live input stream
...
; Mix   -  mix between the signal after the Lock module and the output of the Blur module
...
<Cabbage>
form caption("Spectral Etcher") size(1225,260), colour(0,0,0) pluginId("SpEt"), guiMode("queue")
image                 bounds(  0,  0,1225,260), colour(50,50,60), outlineColour("White"), line(3), shape("sharp")

soundfiler            bounds(  5,  5,605,150), channel("beg","len"), channel("filer1"),  colour(0, 255, 255, 255), fontColour(160, 160, 160, 255)
image                 bounds(614,  5,600,150), colour("black")
signaldisplay         bounds(614,  5,600,150), colour("cornflowerblue"), channel("fft"), displayType("spectrogram"), signalVariable("asig","ascan")
label                 bounds(  6,  4,990, 14), text(""), align(left), colour(0,0,0,0), fontColour(200,200,200), channel("stringbox")

button                bounds(1215,  5, 10, 50), text("",""), channel("SpecHigh"), value(0), latched(1), radioGroup(2), colour:1(200,200,100)
button                bounds(1215, 55, 10, 50), text("",""), channel("SpecMed"),  value(0), latched(1), radioGroup(2), colour:1(200,200,100)
button                bounds(1215,105, 10, 50), text("",""), channel("SpecLow"),  value(1), latched(1), radioGroup(2), colour:1(200,200,100)

nslider               bounds(615,128, 60, 25), text("Fundamental"), channel("Fund"),  range(0,20000,0,1,0.01), alpha(0.5)              $RSliderStyle
...
rslider  bounds(165,160, 70, 90), text("Pitch"),  channel("Pitch"),  range(0.03125,2,1,1,0.00001),              $RSliderStyle
...
rslider  bounds(1055,160, 70, 90), text("Mix"),  channel("Mix"),  range(0,1,1),              $RSliderStyle
rslider  bounds(1125,160, 70, 90), text("Gain"),  channel("Gain"),  range(0,50,1,0.5,0.001),              $RSliderStyle

vmeter   bounds(1205,160, 10, 85) channel("vMeter1") value(0) overlayColour(0, 0, 0, 255) meterColour:0(255, 255, 0) meterColour:1(255, 103, 171) meterColour:2(250,250, 0) outlineThickness(1)

label    bounds(1134,251,120, 10), text("Iain McCurdy |2021|"), align("left"), fontColour("silver")
</Cabbage>
...
kFFTSize init              4
if changed:k(kFFTSize)==1 && kPlayLoop==1 then
             event    "i", 2, 0, -1
endif 
...
; Mix
f1       pvsgain    fLock, 1-gkMix
f2       pvsgain    fBlur, gkMix
fMix     pvsmix     f1, f2
...
; Mix
f1       pvsgain    fLockR, 1-gkMix
f2       pvsgain    fBlurR, gkMix
fMixR    pvsmix     f1, f2
...
; Mix
f1L      pvsgain    fLockL, 1-gkMix
f2L      pvsgain    fBlurL, gkMix
fMixL    pvsmix     f1L, f2L
f1R      pvsgain    fLockR, 1-gkMix
f2R      pvsgain    fBlurR, gkMix
fMixR    pvsmix     f1R, f2R
