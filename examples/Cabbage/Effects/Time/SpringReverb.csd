...
<Cabbage>
form caption("Spring Reverb") size(730,335), colour( 20, 20, 25) pluginId("Sprg"), guiMode("queue")
...
; OUTPUT
groupbox  bounds(540, 10,180,110), text("O U T P U T") {
rslider   bounds( 20, 35, 60, 60), text("Dry/Wet"), channel("Mix"), range(0,1,0.5) $RSLIDER_ATTRIBUTES
rslider   bounds(100, 35, 60, 60), text("Level"), channel("Level"), range(0,1,0.5) $RSLIDER_ATTRIBUTES
}

; FEEDBACK FILTER
groupbox  bounds( 10,130,420,110), text("F E E D B A C K   F I L T E R") {
...
image     bounds(289, 34,112, 62), colour(0,0,0,0), outlineThickness(2), outlineColour("Grey"), corners(5)
gentable  bounds(290, 35,110, 60), tableNumber(11), channel("DampingID"), zoom(-1), ampRange(0,1.1,11), tableColour("lightBlue"), fill(0), outlineThickness(1)
}

; BOING
groupbox  bounds(440,130,280,110), text("B O I N G") {
...
image     bounds(149, 34,112, 62), colour(0,0,0,0), outlineThickness(2), outlineColour("Grey"), corners(5)
gentable  bounds(150, 35,110, 60), tableNumber(12), channel("BoingID"), zoom(-1), ampRange(0,4.4,12), tableColour("lightBlue"), fill(0), outlineThickness(1)
}

; SPRINGS GRAPHIC
image     bounds( 10, 250,710, 70), colour( 0, 0, 0,255), outlineThickness(4), corners(8) outlineColour("silver") {
gentable  bounds(  5,   5,700, 20), tableNumber(1), fill(0), zoom(-1), ampRange(-1,1,1), tableColour("silver"), tableGridColour(0,0,0,0), channel("Spring1ID")
gentable  bounds(  5,  25,700, 20), tableNumber(2), fill(0), zoom(-1), ampRange(-1,1,2), tableColour("silver"), tableGridColour(0,0,0,0), channel("Spring2ID")
gentable  bounds(  5,  45,700, 20), tableNumber(3), fill(0), zoom(-1), ampRange(-1,1,3), tableColour("silver"), tableGridColour(0,0,0,0), channel("Spring3ID"), visible(0)
}

label     bounds(  10,321, 110, 12), text("Iain McCurdy |2021|")
</Cabbage>
...
i_  ftgen $N, 0, 1024, -9, i(kTime$N)*100+40, iAmp$N, 0 ; function table for spring. Harmonic number is related to delay time
rireturn
...
if kInput!=3 then ; if dual stereo has *not* been selected (option 1 or 2)
                  outs            amixL * kLevel, amixL * kLevel
 ...
 kVib1             limit           (gauss:k(20)*kVib^0.25)-1,0,10
 kVib2             limit           (gauss:k(20)*kVib^0.25)-1,0,10
 kVib3             limit           (gauss:k(20)*kVib^0.25)-1,0,10
 cabbageSet k(1), "Spring1ID", "bounds", 5, 5 + kVib1, 700, 20
 cabbageSet k(1), "Spring2ID", "bounds", 5,25 + kVib2, 700, 20
 cabbageSet k(1), "Spring3ID", "bounds", 5,45 + kVib3, 700, 20
