...
<Cabbage>
form caption("Reverse") size(455,110), guiMode("queue"), pluginId("rvrs"), colour(57,70,76)
image    bounds(0, 0, 455, 95), colour("darkslategrey"), shape("rounded"), outlineColour("white"), outlineThickness(4)

#define SLIDER_DESIGN colour(37,59,59)   textColour(255,255,200), trackerColour("lightblue"), trackerInsideRadius(0.8)

rslider  bounds( 10, 10,  75, 75), text("Time L"),    channel("timeL"),   range(0.010, 4, 1, 0.5,0.001), $SLIDER_DESIGN, valueTextBox(1), fontColour("white"), textBoxOutlineColour("silver")
rslider  bounds( 80, 10,  75, 75), text("Time R"),    channel("timeR"),   range(0.010, 4, 1, 0.5,0.001), $SLIDER_DESIGN, valueTextBox(1), fontColour("white"), textBoxOutlineColour("silver")
rslider  bounds(150, 10, 75, 75),  text("Spread"),    channel("spread"),  range(0, 1.00, 1), $SLIDER_DESIGN
rslider  bounds(220, 10, 75, 75),  text("Mix"),       channel("mix"),     range(0, 1.00, 1), $SLIDER_DESIGN
rslider  bounds(290, 10, 75, 75),  text("Level"),     channel("level"),   range(0, 1.00, 1, 0.5), $SLIDER_DESIGN
...
label     bounds( 1, 97, 110, 12), text("Iain McCurdy |2012|")
</Cabbage>
...
opcode    Reverse, a, aKkkk                   ; nb. CAPITAL K CREATES A K-RATE VARIABLE THAT HAS A USEFUL VALUE ALSO AT I-TIME
    ain,ktime,kreverse,kforward,klink    xin  ; READ IN INPUT ARGUMENTS
    ; four windowing envelopes. An appropriate one will be chosen based on the reversed chunk duration. Each has a different attack and decay time
    ...
    if ktime<0.2 then                           ; IF CHUNK TIME IS LESS THAN 0.2... (VERY SHORT) 
     aenv    table3    aptr,ienv4,1             ; CREATE AMPLITUDE ENVELOPE based on the envelope function table with short attack and decay times
    elseif ktime<0.4 then
     aenv    table3    aptr,ienv3,1
    elseif ktime<2 then
     aenv    table3    aptr,ienv2,1
    else                                        ; longest delay times uses the envelope function table with longest attack and decay times
     aenv    table3    aptr,ienv1,1
    endif
    aptr     =         aptr*atime               ; SCALE PHASOR ACCORDING TO THE LENGTH OF THE DELAY TIME CHOSEN BY THE USER
 
    abuffer  delayr    4                        ; CREATE A DELAY BUFFER
    ...
    afwd     deltap3   atime                    ; FORWARD DELAY
             ...
             xout      (abwd*aenv*kreverse)+(afwd*kforward)  ; SEND AUDIO BACK TO CALLER INSTRUMENT - both forward and reverse delays.
                                                             ; whether these are heard depends on the toggle buttons in the GUI (kreverse and kforward)
                                                             ; APPLY AMPLITUDE ENVELOPE TO REVERSED DELAY TO PREVENT CLICKS.
endop


instr 1
ktimeL       cabbageGetValue    "timeL"
ktimeR       cabbageGetValue    "timeR"
kspread      cabbageGetValue    "spread"
kmix         cabbageGetValue    "mix"
klevel       cabbageGetValue    "level"
kreverse     cabbageGetValue    "reverse"
kforward     cabbageGetValue    "forward"
kTMod        cabbageGetValue    "TMod"
kPMod        cabbageGetValue    "PMod"

/* LINK */
klink        cabbageGetValue    "link"            ; if 'Link L&R' is selected
if klink=1&&kTMod=0 then        
 ktrigL      changed            ktimeL,klink
 ktrigR      changed            ktimeR
 if ktrigL=1 then
             cabbageSetValue    "timeR",ktimeL
 elseif ktrigR=1 then
             cabbageSetValue    "timeL",ktimeR
 endif
endif
