...
<Cabbage>
form caption("Peeaaoooh") size(650,247), pluginId("Peao"), guiMode("queue")
image pos(0, 0),          size(650,247), colour( 30, 40, 43), shape("sharp"), outlineColour("white"), outlineThickness(0) 

#define SLIDER_DESIGN colour( 20, 60, 70), trackerColour(200,240,250), textColour("white"), valueTextBox(1), fontColour("white"), markerColour("silver")

rslider   bounds( 10, 10, 70, 90),  channel("Base"), text("Base"), range(0.5, 1000, 50, 0.5, 0.001), $SLIDER_DESIGN
rslider   bounds( 80, 10, 70, 90),  channel("Layers"), text("Layers"), range(1, 60, 8,1,1), $SLIDER_DESIGN
rslider   bounds(150, 10, 70, 90),  channel("Interval"), text("Interval"), range(0.001, 1, 0.07,0.5,0.001), $SLIDER_DESIGN
rslider   bounds(220, 10, 70, 90),  channel("Shape"), text("Shape"), range(0, 3, 0,1,0.001), $SLIDER_DESIGN
rslider   bounds(290, 10, 70, 90),  channel("Scatter"), text("Scatter"), range(0, 1, 0,0.5,0.0001), $SLIDER_DESIGN
rslider   bounds(360, 10, 70, 90),  channel("Spread"), text("Spread"), range(0, 1, 0), $SLIDER_DESIGN
rslider   bounds(430, 10, 70, 90),  channel("Feedback"), text("Feedback"), range(0, 1.5, 0.95), $SLIDER_DESIGN
checkbox  bounds(430,112, 80, 14),  channel("Overload"), text("Overload"), colour(255, 60, 60), fontColour:0("white"), fontColour:1("white"), shape("Ellipse"), active(0)
rslider   bounds(500, 10, 70, 90),  channel("HPF"),   text("HPF"), range(20,20000,20,0.5,1), $SLIDER_DESIGN
rslider   bounds(570, 10, 70, 90),  channel("LPF"),   text("LPF"), range(20,20000,20000,0.5,1), $SLIDER_DESIGN

image     bounds( 20,120,235,120), outlineColour("silver"), outlineThickness(2), colour(50,50,50,100), corners(10)
{
...
rslider   bounds( 85, 22, 70, 90),  channel("PostHPF"),   text("HPF"), range(20,20000,20,0.5,1), $SLIDER_DESIGN
rslider   bounds(155, 22, 70, 90),  channel("PostLPF"),   text("LPF"), range(20,20000,20000,0.5,1), $SLIDER_DESIGN
}

checkbox  bounds(280,132,110, 15), text("Test Clicks"), channel("TestNoise"), fontColour:0("white"), fontColour:1("white"), colour:0(30,90,30)
label     bounds(280,177,110, 12), text("Separation Mode"), fontColour("white") 
combobox  bounds(280,190,110, 20),  channel("SepMode"),text("Linear","Exponential"), textColour("white"), value(1)

rslider   bounds(480,130, 70, 90),  channel("Mix"),text("Mix"), range(0, 1, 1), $SLIDER_DESIGN
rslider   bounds(560,130, 70, 90),  channel("Level"),text("Level"), range(0, 2, 1,0.5,0.001), $SLIDER_DESIGN

label     bounds(536,233,110, 12), text("Iain McCurdy |2015|"), fontColour(200,200,200), align("right")
...
instr    1
 ...
 kSepMode        cabbageGetValue        "SepMode"
 kPostHPF        cabbageGetValue        "PostHPF"
 kPostLPF        cabbageGetValue        "PostLPF"
 kMix            cabbageGetValue        "Mix"
 kPrePostFilter  cabbageGetValue        "PrePostFilter"
 kTestNoise      cabbageGetValue        "TestNoise"
 
 if kTestNoise==1 then
  aL             dust2         5, randomi:k(0.5,2,1,1)
  aR             dust2         5, randomi:k(0.5,2,1,1)
  aL             butlp         aL,cpsoct(randomh:k(7,14,10))
  aR             butlp         aR,cpsoct(randomh:k(7,14,10))
 else
  aL,aR          ins
 endif
 ...
 if trigger:k(kFeedback,1,0)==1 then        ; trigger to turn red
                 cabbageSet     k(1), "FeedbackID","colour", 170, 60, 70    ; send message
 elseif trigger:k(kFeedback,1,1)==1 then    ; trigger to turn back to normal
                 cabbageSet     k(1), "FeedbackID","colour", 20, 60, 70    ; send message
 endif
 ...
 kOverload       cabbageGetValue "Overload"       ; read in value of 'Overload' LED
 if kRMS>0.9 then                                 ; if rms exceeds a given value...
                 cabbageSetValue "Overload",kOn   ; turn on 'Overload' LED
                 cabbageSetValue "Feedback",(kFeedback - (0.0003)) - ((kFeedback-1)*0.001) ; decrement 'Feedback' slider. This will be repeated every k-cycle until rms is back below the threshold value prescribed above.
 elseif kOverload==1 then                         ; otherwise..., i.e. if 'Overload' LED is on but rms is beneath the threshold...  
                 cabbageSetValue "Overload",kOff  ; turn 'overload' LED off
 endif    
