...
; Input Gain        - gain applied to the input signal. Will also increase the amount of clipping
; Limit             - amplitude limit of the clipping function
; Argument          - a value based upon which Bram de Jong method's transfer function is shaped
; Method            - clipping method:
;                     1. Bram de Jong
;                     2. Sine
;                     3. Tanh
; Gain Compensation - if active, loss of gain on account of clipping is compensated for 
;                      by appropriate gain being applied at the output.
; Level             -  output level
; Transfer Function - display of the clipping function

<Cabbage>
form             size(525,105), caption("clip"), pluginId("Clip"), guiMode("queue")
image    bounds(0, 0, 525,105), colour( 20, 30, 40), shape("rounded"), outlineColour("white"), outlineThickness(4) 
...
label    bounds(  5, 90,110, 12), text("Iain McCurdy |2012|"), align("left")
</Cabbage>
