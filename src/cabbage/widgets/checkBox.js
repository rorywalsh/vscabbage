...
export class Checkbox {
  ...
  getInnerHTML() {
    ...
    const textColor = this.props.style.fontColor || currentState.textColor;
  ...
  <rect x="${checkboxX}" y="${(this.props.bounds.height - checkboxSize) / 2}" width="${checkboxSize}" height="${checkboxSize}" fill="${currentState.backgroundColor}" stroke="${currentState.borderColor}" stroke-width="${currentState.borderWidth}" rx="${this.props.style.borderRadius}" ry="${this.props.style.borderRadius}"></rect>
        <text x="${textX}" y="${this.props.bounds.height / 2}" font-family="${this.props.style.fontFamily}" font-size="${fontSize}" fill="${textColor}" text-anchor="${adjustedTextAnchor}" alignment-baseline="middle">${this.props.label.text}</text>
      </svg>
    `;
  }
}
