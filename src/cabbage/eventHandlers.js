...
export function setupFormHandlers() {
    ...
    // Mouse state tracking for performance mode
    let mouseButtons = {
        left: false,
        right: false,
        middle: false
    ...
    // Mouse move handler - tracks position and sends to Csound when in performance mode (nonDraggable)
    const handleMouseMove = (event) => {
        if (cabbageMode === 'nonDraggable') {
            // Get mouse position relative to the main form
            const rect = form.getBoundingClientRect();
            let x = event.clientX - rect.left;
            let y = event.clientY - rect.top;

            // Clamp positions to the main form bounds
            x = Math.max(0, Math.min(x, rect.width));
            y = Math.max(0, Math.min(y, rect.height));

            // console.log(`Cabbage: Mouse move - x: ${x}, y: ${y}, mode: ${cabbageMode}`);
            // Send mouse position data to Csound channels
            Cabbage.sendChannelData('MOUSE_X', x, vscode);
            Cabbage.sendChannelData('MOUSE_Y', y, vscode);
        } else {
            console.log(`Cabbage: Mouse move ignored - mode is ${cabbageMode}, not 'nonDraggable'`);
        }
    };

    // Mouse down handler - tracks button presses
    const handleMouseDown = (event) => {
        if (cabbageMode === 'nonDraggable') {
            console.log(`Cabbage: Mouse down - button: ${event.button}, mode: ${cabbageMode}`);
            switch (event.button) {
                case 0: // Left button
                    mouseButtons.left = true;
                    Cabbage.sendChannelData('MOUSE_DOWN_LEFT', 1, vscode);
                    break;
                case 1: // Middle button
                    mouseButtons.middle = true;
                    Cabbage.sendChannelData('MOUSE_DOWN_MIDDLE', 1, vscode);
                    break;
                case 2: // Right button
                    mouseButtons.right = true;
                    Cabbage.sendChannelData('MOUSE_DOWN_RIGHT', 1, vscode);
                    break;
            }
        } else {
            console.log(`Cabbage: Mouse down ignored - mode is ${cabbageMode}, not 'nonDraggable'`);
        }
    };

    // Mouse up handler - tracks button releases
    const handleMouseUp = (event) => {
        if (cabbageMode === 'nonDraggable') {
            console.log(`Cabbage: Mouse up - button: ${event.button}, mode: ${cabbageMode}`);
            switch (event.button) {
                case 0: // Left button
                    mouseButtons.left = false;
                    Cabbage.sendChannelData('MOUSE_DOWN_LEFT', 0, vscode);
                    break;
                case 1: // Middle button
                    mouseButtons.middle = false;
                    Cabbage.sendChannelData('MOUSE_DOWN_MIDDLE', 0, vscode);
                    break;
                case 2: // Right button
                    mouseButtons.right = false;
                    Cabbage.sendChannelData('MOUSE_DOWN_RIGHT', 0, vscode);
                    break;
            }
        } else {
            console.log(`Cabbage: Mouse up ignored - mode is ${cabbageMode}, not 'nonDraggable'`);
        }
    };

    // Add mouse event listeners to the main form for performance mode
    if (form) {
        ...
        // Also listen for mouse leave to reset button states when mouse leaves the form
        form.addEventListener('mouseleave', () => {
            if (cabbageMode === 'nonDraggable') {
                console.log('Cabbage: Mouse left form, resetting button states');
                // Reset all button states when mouse leaves the form
    ...
    console.log('Cabbage: Form event handlers set up for mouse tracking');
}